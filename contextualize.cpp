...
namespace Sass {
  ...
  Selector* Contextualize::operator()(Selector_Schema* s)
  {
    ...
    Selector* result_sel = Parser::from_c_str(result_str.c_str(), ctx, s->path(), s->position(), s->closure()).parse_selector_group();
  ...
  Selector* Contextualize::operator()(Selector_List* s)
  {
    ...
    Selector_List* ss = 0;
    if (p) {
      ss = new (ctx.mem) Selector_List(s->path(), s->position(), s->closure(), p->length() * s->length());
      ...
      ss = new (ctx.mem) Selector_List(s->path(), s->position(), s->closure(), s->length());
      for (size_t j = 0, L = s->length(); j < L; ++j) {
        Complex_Selector* comb = static_cast<Complex_Selector*>((*s)[j]->perform(this));
        if (comb) *ss << comb;
      }
  ...
  Selector* Contextualize::operator()(Compound_Selector* s)
  {
    ...
    Compound_Selector* ss = new (ctx.mem) Compound_Selector(s->path(), s->position(), s->closure(), s->length());
