...
namespace Sass {
  ...
  Selector* Contextualize::operator()(Selector_Schema* s)
  {
    ...
    Selector* result_sel = Parser::from_c_str(result_str.c_str(), ctx, s->slct().path, s->slct().position).parse_selector_group();
  ...
  Selector* Contextualize::operator()(Selector_List* s)
  {
    ...
    Selector_List* ss = 0;
    if (p) {
      ss = new (ctx.mem) Selector_List(s->slct(), p->length() * s->length());
      ...
      ss = new (ctx.mem) Selector_List(s->slct(), s->length());
      for (size_t j = 0, L = s->length(); j < L; ++j) {
        Complex_Selector* comb = static_cast<Complex_Selector*>((*s)[j]->perform(this));
        if (comb) *ss << comb;
      }
  ...
  Selector* Contextualize::operator()(Compound_Selector* s)
  {
    ...
    Compound_Selector* ss = new (ctx.mem) Compound_Selector(s->slct(), s->length());
  ...
  Selector* Contextualize::operator()(Wrapped_Selector* s)
  {
    ...
    parent = 0;
    Wrapped_Selector* neg = new (ctx.mem) Wrapped_Selector(s->slct(),
                                                           s->name(),
