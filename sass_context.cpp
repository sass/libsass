  ...
  static int handle_errors(Sass_Context* c_ctx) {
    try {
     throw;
      ...
      json_append_member(json_err, "line", json_mknumber(e.slct.line));
      json_append_member(json_err, "column", json_mknumber(e.slct.column));
      json_append_member(json_err, "message", json_mkstring(e.message.c_str()));
      msg_stream << e.slct.path << ":" << e.slct.line << ": " << e.message << endl;
      ...
      c_ctx->error_line = e.slct.line;
      c_ctx->error_column = e.slct.column;
