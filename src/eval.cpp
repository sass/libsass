...
namespace Sass
{
  ...
  bop ops[Sass_OP::NUM_OPS] = { 0,   0, // and, or
                                0,   0,   0,   0,   0,  0, // eq, neq, gt, gte, lt, lte
                                add, sub, mul, div, mod };

  Eval::Eval(Expand& exp) : exp(exp), ctx(exp.ctx), force(false), is_in_comment(false)
  {
  ...
  Expression_Ptr Eval::operator()(For_Ptr f)
  {
    ...
    if (sass_start->unit() != sass_end->unit())
    {
      ...
      msg << "Incompatible units: '" << sass_end->unit() << "' and '" << sass_start->unit() << "'.";
      ...
      for (size_t i = 0, L = list->length(); i < L; ++i)
      {
        ...
        if (List_Ptr scalars = Cast<List>(item))
        {
          ...
          else
          {
            ...
            {
              Expression_Ptr res =
              j >= scalars->length() ? SASS_MEMORY_NEW(Null, expr->pstate()) : scalars->at(j);
  ...
  Expression_Ptr Eval::operator()(Warning_Ptr w)
  {
    ...
    if (env->has("@warn[f]"))
    {
      ...
      { "@warn", w->pstate().path, w->pstate().line + 1, w->pstate().column + 1, SASS_CALLEE_FUNCTION, { env } });
  ...
  Expression_Ptr Eval::operator()(Error_Ptr e)
  {
    ...
    if (env->has("@error[f]"))
    {
      ...
      // add call stack entry
      ctx.callee_stack.push_back(
      { "@error", e->pstate().path, e->pstate().line + 1, e->pstate().column + 1, SASS_CALLEE_FUNCTION, { env } });
  ...
  Expression_Ptr Eval::operator()(Debug_Ptr d)
  {
    ...
    if (env->has("@debug[f]"))
    {
      ...
      // add call stack entry
      ctx.callee_stack.push_back(
      { "@debug", d->pstate().path, d->pstate().line + 1, d->pstate().column + 1, SASS_CALLEE_FUNCTION, { env } });
  ...
  Expression_Ptr Eval::operator()(List_Ptr l)
  {
    ...
    {
      Map_Obj lm = SASS_MEMORY_NEW(Map, l->pstate(), l->length() / 2);
    ...
    List_Obj ll =
    SASS_MEMORY_NEW(List, l->pstate(), l->length(), l->separator(), l->is_arglist(), l->is_bracketed());
  ...
  Expression_Ptr Eval::operator()(Map_Ptr m)
  {
    ...
    Map_Obj mm = SASS_MEMORY_NEW(Map, m->pstate(), m->length());
    for (auto key : m->keys())
    {
  ...
  Expression_Ptr Eval::operator()(Binary_Expression_Ptr b_in)
  {
    ...
    if (Number_Ptr l_n = Cast<Number>(lhs))
    {
      ...
      if (Number_Ptr r_n = Cast<Number>(rhs))
      {
        try
        {
          ...
          case Sass_OP::NEQ:
            return *l_n == *r_n ? bool_false : bool_true;
          case Sass_OP::LT:
            return *l_n < *r_n ? bool_true : bool_false;
          case Sass_OP::GTE:
            return *l_n < *r_n ? bool_false : bool_true;
          case Sass_OP::LTE:
            return *l_n < *r_n || *l_n == *r_n ? bool_true : bool_false;
          case Sass_OP::GT:
            return *l_n < *r_n || *l_n == *r_n ? bool_false : bool_true;
          case Sass_OP::ADD:
          case Sass_OP::SUB:
          case Sass_OP::MUL:
          case Sass_OP::DIV:
      ...
      else if (Color_Ptr r_c = Cast<Color>(rhs))
      {
        try
        {
          ...
          case Sass_OP::NEQ:
            return *l_n == *r_c ? bool_false : bool_true;
          case Sass_OP::LT:
            return *l_n < *r_c ? bool_true : bool_false;
          case Sass_OP::GTE:
            return *l_n < *r_c ? bool_false : bool_true;
          case Sass_OP::LTE:
            return *l_n < *r_c || *l_n == *r_c ? bool_true : bool_false;
          case Sass_OP::GT:
            return *l_n < *r_c || *l_n == *r_c ? bool_false : bool_true;
          case Sass_OP::ADD:
          case Sass_OP::SUB:
          case Sass_OP::MUL:
          case Sass_OP::DIV:
    ...
    else if (Color_Ptr l_c = Cast<Color>(lhs))
    {
      ...
      if (Color_Ptr r_c = Cast<Color>(rhs))
      {
        try
        {
          ...
          case Sass_OP::NEQ:
            return *l_c == *r_c ? bool_false : bool_true;
          case Sass_OP::LT:
            return *l_c < *r_c ? bool_true : bool_false;
          case Sass_OP::GTE:
            return *l_c < *r_c ? bool_false : bool_true;
          case Sass_OP::LTE:
            return *l_c < *r_c || *l_c == *r_c ? bool_true : bool_false;
          case Sass_OP::GT:
            return *l_c < *r_c || *l_c == *r_c ? bool_false : bool_true;
          case Sass_OP::ADD:
          case Sass_OP::SUB:
          case Sass_OP::MUL:
          case Sass_OP::DIV:
      ...
      else if (Number_Ptr r_n = Cast<Number>(rhs))
      {
        try
        {
          ...
          case Sass_OP::NEQ:
            return *l_c == *r_n ? bool_false : bool_true;
          case Sass_OP::LT:
            return *l_c < *r_n ? bool_true : bool_false;
          case Sass_OP::GTE:
            return *l_c < *r_n ? bool_false : bool_true;
          case Sass_OP::LTE:
            return *l_c < *r_n || *l_c == *r_n ? bool_true : bool_false;
          case Sass_OP::GT:
            return *l_c < *r_n || *l_c == *r_n ? bool_false : bool_true;
          case Sass_OP::ADD:
          case Sass_OP::SUB:
          case Sass_OP::MUL:
          case Sass_OP::DIV:
    ...
    if (String_Schema_Ptr s_l = Cast<String_Schema>(b->left()))
    {
      if (!s_l->has_interpolant() && (!s_l->is_right_interpolant()))
      {
        ...
        Binary_Expression_Obj bin_ex =
        SASS_MEMORY_NEW(Binary_Expression, b->pstate(), b->op(), s_l->last(), b->right());
    ...
    if (String_Schema_Ptr s_r = Cast<String_Schema>(b->right()))
    {
      ...
      if (!s_r->has_interpolant() && (!s_r->is_left_interpolant() || op_type == Sass_OP::DIV))
      {
        ret_schema = SASS_MEMORY_NEW(String_Schema, b->pstate());
        Binary_Expression_Obj bin_ex =
        SASS_MEMORY_NEW(Binary_Expression, b->pstate(), b->op(), b->left(), s_r->first());
    ...
    if (op_type == Sass_OP::EQ || op_type == Sass_OP::NEQ || op_type == Sass_OP::GT ||
        op_type == Sass_OP::GTE || op_type == Sass_OP::LT || op_type == Sass_OP::LTE)
    {
      LOCAL_FLAG(force, true);
    ...
    bool force_delay = (s2 && s2->is_left_interpolant()) || (s1 && s1->is_right_interpolant()) ||
                       (b1 && b1->is_right_interpolant()) || (b2 && b2->is_left_interpolant());

    if ((s1 && s1->has_interpolants()) || (s2 && s2->has_interpolants()) || force_delay)
    {
      if (op_type == Sass_OP::DIV || op_type == Sass_OP::MUL || op_type == Sass_OP::MOD ||
          op_type == Sass_OP::ADD || op_type == Sass_OP::SUB || op_type == Sass_OP::EQ)
      {
    ...
    try
    {
      ...
      {
      case Sass_OP::EQ:
        return SASS_MEMORY_NEW(Boolean, b->pstate(), eq(lhs, rhs));
      case Sass_OP::NEQ:
        return SASS_MEMORY_NEW(Boolean, b->pstate(), !eq(lhs, rhs));
      case Sass_OP::GT:
        return SASS_MEMORY_NEW(Boolean, b->pstate(), !lt(lhs, rhs, "gt") && !eq(lhs, rhs));
      case Sass_OP::GTE:
        return SASS_MEMORY_NEW(Boolean, b->pstate(), !lt(lhs, rhs, "gte"));
      case Sass_OP::LT:
        return SASS_MEMORY_NEW(Boolean, b->pstate(), lt(lhs, rhs, "lt"));
      case Sass_OP::LTE:
        return SASS_MEMORY_NEW(Boolean, b->pstate(), lt(lhs, rhs, "lte") || eq(lhs, rhs));
      default:
        break;
      }
    }
    ...
    try
    {
      ...
      else
      {
        ...
        Value_Obj v_l = Cast<Value>(lhs->perform(&to_value));
        Value_Obj v_r = Cast<Value>(rhs->perform(&to_value));
        bool interpolant = b->is_right_interpolant() || b->is_left_interpolant() || b->is_interpolant();
        ...
        Value_Ptr ex = op_strings(b->op(), *v_l, *v_r, ctx.c_options, pstate,
                                  !interpolant); // pass true to compress
  ...
  Expression_Ptr Eval::operator()(Unary_Expression_Ptr u)
  {
    ...
    else
    {
      ...
      return SASS_MEMORY_NEW(String_Quoted, u->pstate(), u->inspect());
    }
    // unreachable
    return u;
  }
  ...
  Expression_Ptr Eval::operator()(Function_Call_Ptr c)
  {
    ...
    if (!env->has(full_name) || (!c->via_call() && Prelexer::re_special_fun(name.c_str())))
    {
      if (!env->has("*[f]"))
      {
        ...
        Function_Call_Obj lit = SASS_MEMORY_NEW(Function_Call, c->pstate(), c->name(), args);
        if (args->has_named_arguments())
        {
          error("Function " + c->name() + " doesn't support keyword arguments", c->pstate());
        }
        String_Quoted_Ptr str = SASS_MEMORY_NEW(String_Quoted, c->pstate(), lit->to_string(ctx.c_options));
    ...
    if (def->is_overload_stub())
    {
      ...
      if (!env->has(resolved_name))
        error("overloaded function `" + std::string(c->name()) +
    ...
    if (func || body)
    {
      ...
      ctx.callee_stack.push_back(
      { c->name().c_str(), c->pstate().path, c->pstate().line + 1, c->pstate().column + 1, SASS_CALLEE_FUNCTION, { env } });
    ...
    else if (c_function)
    {
      ...
      exp.backtrace_stack.push_back(&here);
      ctx.callee_stack.push_back(
      { c->name().c_str(), c->pstate().path, c->pstate().line + 1, c->pstate().column + 1, SASS_CALLEE_C_FUNCTION, { env } });
      ...
      {
        error("error in C function " + c->name() + ": " + sass_error_get_message(c_val),
              c->pstate(), backtrace());
      }
      else if (sass_value_get_tag(c_val) == SASS_WARNING)
      {
        error("warning in C function " + c->name() + ": " + sass_warning_get_message(c_val),
              c->pstate(), backtrace());
      }
      ...
      ctx.callee_stack.pop_back();
      sass_delete_value(c_args);
      if (c_val != c_args) sass_delete_value(c_val);
    }

    // link back to function definition
    // only do this for custom functions
    if (result->pstate().file == std::string::npos) result->pstate(c->pstate());
  ...
  Expression_Ptr Eval::operator()(Variable_Ptr v)
  {
    ...
    if (rv.found)
      value = static_cast<Expression*>(rv.it->second.ptr());
    else
      error("Undefined variable: \"" + v->name() + "\".", v->pstate());
  ...
  void Eval::interpolation(Context& ctx, std::string& res, Expression_Obj ex, bool into_quotes, bool was_itpl)
  {
    ...
    if (List_Ptr l = Cast<List>(ex))
    {
      ...
      for (Expression_Obj item : *l)
      {
        ...
        std::string rl("");
        interpolation(ctx, rl, item, into_quotes, l->is_interpolant());
  ...
  Expression_Ptr Eval::operator()(String_Schema_Ptr s)
  {
      ...
      if (!Cast<String_Quoted>((*s)[0]) && !Cast<String_Quoted>((*s)[L - 1]))
      {
        if (String_Constant_Ptr l = Cast<String_Constant>((*s)[0]))
        {
          if (String_Constant_Ptr r = Cast<String_Constant>((*s)[L - 1]))
          {
            if (r->value().size() > 0)
            {
              if (l->value()[0] == '"' && r->value()[r->value().size() - 1] == '"')
                into_quotes = true;
              if (l->value()[0] == '\'' && r->value()[r->value().size() - 1] == '\'')
                into_quotes = true;
            }
    ...
    for (size_t i = 0; i < L; ++i)
    {
      ...
      if (was_quoted && !(*s)[i]->is_interpolant() && !was_interpolant)
      {
        res += " ";
      }
      else if (i > 0 && is_quoted && !(*s)[i]->is_interpolant() && !was_interpolant)
      {
        res += " ";
      }
    ...
    if (str->quote_mark())
      ...
      str->value(string_to_output(str->value()));
  ...
  Expression_Ptr Eval::operator()(Supports_Operator_Ptr c)
  {
    ...
    Supports_Operator_Ptr cc = SASS_MEMORY_NEW(Supports_Operator, c->pstate(), Cast<Supports_Condition>(left),
                                               Cast<Supports_Condition>(right), c->operand());
    return cc;
  }
  ...
  Expression_Ptr Eval::operator()(Supports_Negation_Ptr c)
  {
    ...
    Supports_Negation_Ptr cc =
    SASS_MEMORY_NEW(Supports_Negation, c->pstate(), Cast<Supports_Condition>(condition));
    return cc;
  }
  ...
  Expression_Ptr Eval::operator()(Supports_Declaration_Ptr c)
  {
    ...
    Supports_Declaration_Ptr cc = SASS_MEMORY_NEW(Supports_Declaration, c->pstate(), feature, value);
    return cc;
  }
  ...
  {
    Expression_Ptr value = c->value()->perform(this);
    Supports_Interpolation_Ptr cc = SASS_MEMORY_NEW(Supports_Interpolation, c->pstate(), value);
    return cc;
  }
  ...
  Expression_Ptr Eval::operator()(At_Root_Query_Ptr e)
  {
    ...
    Expression_Ptr ee = SASS_MEMORY_NEW(At_Root_Query, e->pstate(), Cast<String>(feature), value);
    return ee;
  }
  ...
  Media_Query_Ptr Eval::operator()(Media_Query_Ptr q)
  {
    ...
    Media_Query_Obj qq =
    SASS_MEMORY_NEW(Media_Query, q->pstate(), t, q->length(), q->is_negated(), q->is_restricted());
  ...
  Expression_Ptr Eval::operator()(Media_Query_Expression_Ptr e)
  {
    ...
    {
      feature = SASS_MEMORY_NEW(String_Quoted, feature->pstate(), Cast<String_Quoted>(feature)->value());
    }
    Expression_Obj value = e->value();
    value = (value ? value->perform(this) : 0);
    ...
    {
      // XXX: this is never hit via spec tests
      value = SASS_MEMORY_NEW(String_Quoted, value->pstate(), Cast<String_Quoted>(value)->value());
    }
    return SASS_MEMORY_NEW(Media_Query_Expression, e->pstate(), feature, value, e->is_interpolated());
  }
  ...
  Expression_Ptr Eval::operator()(Argument_Ptr a)
  {
    ...
    if (a->is_rest_argument())
    {
      ...
      {
        List_Obj wrapper = SASS_MEMORY_NEW(List, val->pstate(), 0, SASS_COMMA, true);
        wrapper->append(val);
        val = wrapper;
      }
    }
    return SASS_MEMORY_NEW(Argument, a->pstate(), val, a->name(), is_rest_argument, is_keyword_argument);
  }
  ...
  Expression_Ptr Eval::operator()(Arguments_Ptr a)
  {
    ...
    if (a->has_rest_argument())
    {
      ...
      List_Obj arglist = SASS_MEMORY_NEW(List, splat->pstate(), 0, ls ? ls->separator() : separator, true);
  ...
  Value_Ptr
  Eval::op_numbers(enum Sass_OP op, const Number& l, const Number& r, struct Sass_Inspect_Options opt, const ParserState& pstate)
  {
    double lv = l.value();
    double rv = r.value();
  ...
  Value_Ptr Eval::op_number_color(enum Sass_OP op,
                                  const Number& l,
                                  const Color& r,
                                  struct Sass_Inspect_Options opt,
                                  const ParserState& pstate)
  {
    ...
    case Sass_OP::MUL:
    {
      return SASS_MEMORY_NEW(Color, pstate, ops[op](lv, r.r()), ops[op](lv, r.g()),
                             ops[op](lv, r.b()), r.a());
    }
    case Sass_OP::SUB:
    case Sass_OP::DIV:
    {
      ...
      return SASS_MEMORY_NEW(String_Quoted, pstate, l.to_string(opt) + sep + color);
    }
    case Sass_OP::MOD:
    {
    ...
    default:
      break; // caller should ensure that we don't get here
    }
    // unreachable
    return NULL;
  }

  Value_Ptr Eval::op_color_number(enum Sass_OP op,
                                  const Color& l,
                                  const Number& r,
                                  struct Sass_Inspect_Options opt,
                                  const ParserState& pstate)
  {
    ...
    return SASS_MEMORY_NEW(Color, pstate, ops[op](l.r(), rv), ops[op](l.g(), rv),
                           ops[op](l.b(), rv), l.a());
  }

  Value_Ptr
  Eval::op_colors(enum Sass_OP op, const Color& l, const Color& r, struct Sass_Inspect_Options opt, const ParserState& pstate)
  {
    if (l.a() != r.a())
    {
    ...
    return SASS_MEMORY_NEW(Color, pstate, ops[op](l.r(), r.r()), ops[op](l.g(), r.g()),
                           ops[op](l.b(), r.b()), l.a());
  }

  Value_Ptr Eval::op_strings(Sass::Operand operand,
                             Value& lhs,
                             Value& rhs,
                             struct Sass_Inspect_Options opt,
                             const ParserState& pstate,
                             bool delayed)
  {
    ...
    if (ltype == Expression::NULL_VAL)
      throw Exception::InvalidNullOperation(&lhs, &rhs, sass_op_to_name(op));
    if (rtype == Expression::NULL_VAL)
      throw Exception::InvalidNullOperation(&lhs, &rhs, sass_op_to_name(op));
    std::string sep;
    switch (op)
    {
    ...
    case Sass_OP::GTE:
      ...
      break;
    case Sass_OP::MUL:
      throw Exception::UndefinedOperation(&lhs, &rhs, sass_op_to_name(op));
    case Sass_OP::MOD:
      throw Exception::UndefinedOperation(&lhs, &rhs, sass_op_to_name(op));
    default:
      break;
  ...
  Expression_Ptr cval_to_astnode(union Sass_Value* v, Backtrace* backtrace, ParserState pstate)
  {
    ...
    case SASS_COLOR:
    {
      e = SASS_MEMORY_NEW(Color, pstate, sass_color_get_r(v), sass_color_get_g(v),
                          sass_color_get_b(v), sass_color_get_a(v));
    }
    break;
    case SASS_STRING:
    {
    ...
    case SASS_MAP:
    {
      ...
      {
        *m << std::make_pair(cval_to_astnode(sass_map_get_key(v, i), backtrace, pstate),
  ...
  Attribute_Selector_Ptr Eval::operator()(Attribute_Selector_Ptr s)
  {
    ...
    if (attr)
    {
      attr = static_cast<String_Ptr>(attr->perform(this));
    }
