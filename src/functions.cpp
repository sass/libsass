...
namespace Sass
{
  ...
  Definition_Ptr make_native_function(Signature sig, Native_Function func, Context& ctx)
  {
    ...
    return SASS_MEMORY_NEW(Definition, ParserState("[built-in function]"), sig, name, params, func, false);
  }
  ...
  Definition_Ptr make_c_function(Sass_Function_Entry c_func, Context& ctx)
  {
    ...
    sig_parser.lex<alternatives<identifier, exactly<'*'>, exactly<Constants::warn_kwd>,
                                exactly<Constants::error_kwd>, exactly<Constants::debug_kwd>>>();
    std::string name(Util::normalize_underscores(sig_parser.lexed));
    Parameters_Obj params = sig_parser.parse_parameters();
    return SASS_MEMORY_NEW(Definition, ParserState("[c function]"), sig, name, params, c_func, false, true);
  }
  ...
  namespace Functions
  {
    ...
    Map_Ptr
    get_arg_m(const std::string& argname, Env& env, Signature sig, ParserState pstate, Backtrace* backtrace, Context& ctx)
    {
      // Minimal error handling -- the expectation is that built-ins will be written correctly!
    ...
    Number_Ptr
    get_arg_r(const std::string& argname, Env& env, Signature sig, ParserState pstate, double lo, double hi, Backtrace* backtrace)
    {
      // Minimal error handling -- the expectation is that built-ins will be written correctly!
...
#define ARGSEL(argname, seltype, contextualize) \
  get_arg_sel<seltype>(argname, env, sig, pstate, backtrace, ctx)
    ...
    template <>
    Selector_List_Obj
    get_arg_sel(const std::string& argname, Env& env, Signature sig, ParserState pstate, Backtrace* backtrace, Context& ctx)
    {
    ...
    // features
    static std::set<std::string> features{ "global-variable-shadowing",
                                           "extend-selector-pseudoclass", "at-error",
                                           "units-level-3" };
    ...
    BUILT_IN(rgb)
    {
      return SASS_MEMORY_NEW(Color, pstate, color_num(ARG("$red", Number)),
                             color_num(ARG("$green", Number)), color_num(ARG("$blue", Number)));
    }

    Signature rgba_4_sig = "rgba($red, $green, $blue, $alpha)";
    BUILT_IN(rgba_4)
    {
      return SASS_MEMORY_NEW(Color, pstate, color_num(ARG("$red", Number)), color_num(ARG("$green", Number)),
                             color_num(ARG("$blue", Number)), alpha_num(ARG("$alpha", Number)));
    }
    ...
    Color* colormix(Context& ctx, ParserState& pstate, Color* color1, Color* color2, Number* weight)
    {
      ...
      return SASS_MEMORY_NEW(Color, pstate,
    ...
    BUILT_IN(hsl)
    {
      return hsla_impl(ARG("$hue", Number)->value(), ARG("$saturation", Number)->value(),
                       ARG("$lightness", Number)->value(), 1.0, ctx, pstate);
    }
    ...
    {
      return hsla_impl(ARG("$hue", Number)->value(), ARG("$saturation", Number)->value(),
                       ARG("$lightness", Number)->value(), ARG("$alpha", Number)->value(), ctx, pstate);
    }
    ...
    BUILT_IN(hue)
    {
      ...
      HSL hsl_color = rgb_to_hsl(rgb_color->r(), rgb_color->g(), rgb_color->b());
    ...
    BUILT_IN(adjust_hue)
    {
      ...
      HSL hsl_color = rgb_to_hsl(rgb_color->r(), rgb_color->g(), rgb_color->b());
      return hsla_impl(hsl_color.h + degrees->value(), hsl_color.s, hsl_color.l, rgb_color->a(), ctx, pstate);
    }
    ...
    BUILT_IN(lighten)
    {
      ...
      return hsla_impl(hsl_color.h, hsl_color.s, hslcolorL + amount->value(), rgb_color->a(), ctx, pstate);
    }
    ...
    BUILT_IN(saturate)
    {
      ...
      if (!amount)
      {
        return SASS_MEMORY_NEW(String_Quoted, pstate,
                               "saturate(" + env["$color"]->to_string(ctx.c_options) + ")");
      }
      ...
      return hsla_impl(hsl_color.h, hslcolorS, hsl_color.l, rgb_color->a(), ctx, pstate);
    }
    ...
    BUILT_IN(grayscale)
    {
      ...
      Color_Ptr rgb_color = ARG("$color", Color);
      HSL hsl_color = rgb_to_hsl(rgb_color->r(), rgb_color->g(), rgb_color->b());
      return hsla_impl(hsl_color.h, 0.0, hsl_color.l, rgb_color->a(), ctx, pstate);
    }
    ...
    BUILT_IN(complement)
    {
      Color_Ptr rgb_color = ARG("$color", Color);
      HSL hsl_color = rgb_to_hsl(rgb_color->r(), rgb_color->g(), rgb_color->b());
      return hsla_impl(hsl_color.h - 180.0, hsl_color.s, hsl_color.l, rgb_color->a(), ctx, pstate);
    }
    ...
    BUILT_IN(invert)
    {
      ...
      Number_Obj weight = ARGR("$weight", Number, 0, 100);
      Color_Ptr rgb_color = ARG("$color", Color);
      Color_Obj inv = SASS_MEMORY_NEW(Color, pstate, 255 - rgb_color->r(), 255 - rgb_color->g(),
                                      255 - rgb_color->b(), rgb_color->a());
    ...
    BUILT_IN(opacify)
    {
      ...
      return SASS_MEMORY_NEW(Color, pstate, color->r(), color->g(), color->b(), alpha);
    }
    ...
    Signature adjust_color_sig = "adjust-color($color, $red: false, $green: false, $blue: false, "
                                 "$hue: false, $saturation: false, $lightness: false, $alpha: "
                                 "false)";
    BUILT_IN(adjust_color)
    {
      ...
      if (rgb)
      {
        ...
        return SASS_MEMORY_NEW(Color, pstate, color->r() + rr, color->g() + gg, color->b() + bb,
                               color->a() + aa);
      }
      if (hsl)
      {
        ...
        double aa = a ? ARGR("$alpha", Number, -1, 1)->value() : 0;
        return hsla_impl(hsl_struct.h + (h ? h->value() : 0), hsl_struct.s + ss, hsl_struct.l + ll,
                         color->a() + aa, ctx, pstate);
      }
      if (a)
      {
        return SASS_MEMORY_NEW(Color, pstate, color->r(), color->g(), color->b(),
                               color->a() + (a ? a->value() : 0));
      }
    ...
    Signature scale_color_sig = "scale-color($color, $red: false, $green: false, $blue: false, "
                                "$hue: false, $saturation: false, $lightness: false, $alpha: "
                                "false)";
    ...
    Signature change_color_sig = "change-color($color, $red: false, $green: false, $blue: false, "
                                 "$hue: false, $saturation: false, $lightness: false, $alpha: "
                                 "false)";
    BUILT_IN(change_color)
    {
      ...
      if (rgb)
      {
        return SASS_MEMORY_NEW(Color, pstate, r ? ARGR("$red", Number, 0, 255)->value() : color->r(),
    ...
    template <size_t range> static double cap_channel(double c)
    {
      if (c > range)
        return range;
      else if (c < 0)
    ...
    BUILT_IN(str_length)
    {
      ...
      {
        handle_utf8_error(pstate, backtrace);
      }
    ...
    BUILT_IN(percentage)
    {
      ...
      if (!n->is_unitless())
        error("argument $number of `" + std::string(sig) + "` must be unitless", pstate);
    ...
    BUILT_IN(length)
    {
      ...
      return SASS_MEMORY_NEW(Number, pstate, (double)(list ? list->size() : 1));
    }
    ...
    BUILT_IN(nth)
    {
      ...
      if (Selector_List_Ptr sl = Cast<Selector_List>(env["$list"]))
      {
        ...
        if (empty)
          error("argument `$list` of `" + std::string(sig) + "` must not be empty", pstate);
        double index = std::floor(n->value() < 0 ? len + n->value() : n->value() - 1);
        if (index < 0 || index > len - 1)
          error("index out of bounds for `" + std::string(sig) + "`", pstate);
      ...
      if (n->value() == 0)
        error("argument `$n` of `" + std::string(sig) + "` must be non-zero", pstate);
    ...
    BUILT_IN(index)
    {
      ...
      for (size_t i = 0, L = l->length(); i < L; ++i)
      {
        if (Eval::eq(l->value_at_index(i), v))
    ...
    BUILT_IN(join)
    {
      ...
      if (sep_str == "space")
        sep_val = SASS_SPACE;
      else if (sep_str == "comma")
        sep_val = SASS_COMMA;
      else if (sep_str != "auto")
        error("argument `$separator` of `" + std::string(sig) +
    ...
    BUILT_IN(append)
    {
      ...
      if (l->is_arglist())
      {
        result->append(SASS_MEMORY_NEW(Argument, v->pstate(), v, "", false, false));
      }
      else
      {
        result->append(v);
    ...
    BUILT_IN(keywords)
    {
      ...
      for (size_t i = arglist->size(), L = arglist->length(); i < L; ++i)
      {
        ...
        *result << std::make_pair(SASS_MEMORY_NEW(String_Quoted, pstate, name), arg->value());
      }
      return result.detach();
    }
    ...
    BUILT_IN(call)
    {
      ...
      for (size_t i = 0, L = arglist->length(); i < L; ++i)
      {
        ...
        if (arglist->is_arglist())
        {
          Expression_Obj obj = arglist->at(i);
          Argument_Obj arg = (Argument_Ptr)obj.ptr(); // XXX
          args->append(SASS_MEMORY_NEW(Argument, pstate, expr, arg ? arg->name() : "",
    ...
    // BUILT_IN(sass_if)
    // { return ARG("$condition", Expression)->is_false() ? ARG("$if-false", Expression) :
    // ARG("$if-true", Expression); }
    BUILT_IN(sass_if)
    {
      Expand expand(ctx, &d_env, backtrace, &selector_stack);
    ...
    BUILT_IN(selector_nest)
    {
      ...
      // Set the first element as the `result`, keep appending to as we go down the parsedSelector
      // vector.
