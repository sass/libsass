    ...
    while (ia < LA)
    {
      ...
      if (p->is_rest_parameter())
      {
        ...
        if (a->is_rest_argument())
        {
          ...
          if (List_Obj rest = Cast<List>(a->value()))
          {
            ...
            List_Ptr arglist = SASS_MEMORY_NEW(List, p->pstate(), 0, rest->separator(), true);
            ...
            for (Expression_Obj item : rest->elements())
            {
              ...
              {
                arglist->append(SASS_MEMORY_NEW(Argument, item->pstate(), item, "", false, false));
              }
            }
        ...
        else if (a->is_keyword_argument())
        {
          ...
          for (auto key : argmap->keys())
          {
            ...
            arglist->append(SASS_MEMORY_NEW(Argument, key->pstate(), argmap->at(key), "$" + param, false, false));
          }
        }
        else
        {
          ...
          // create a new list object for wrapped items
          List_Obj arglist = SASS_MEMORY_NEW(List, p->pstate(), 0, SASS_COMMA, true);
          ...
          while (ia < LA)
          {
            ...
            else if (a->is_rest_argument())
            {
              ...
              if (List_Obj rest = Cast<List>(a->value()))
              {
                ...
                for (size_t i = 0, L = rest->size(); i < L; ++i)
                {
                  ...
                  arglist->append(SASS_MEMORY_NEW(Argument, obj->pstate(), obj, "", false, false));
                }
              }
              // no more arguments
              break;
            ...
            {
              arglist->append(SASS_MEMORY_NEW(Argument, a->pstate(), a->value(), a->name(), false, false));
            }
          }
          // assign new arglist to environment
          env->local_frame()[p->name()] = arglist;
      ...
      else if (a->is_rest_argument())
      {
        ...
        if (!(a = Cast<Argument>(obj)))
        {
          ...
          a = SASS_MEMORY_NEW(Argument, a_to_convert->pstate(), a_to_convert, "", false, false);
        }
      ...
      if (a->name().empty())
      {
        ...
        {
          std::stringstream msg;
          msg << "parameter " << p->name() << " provided more than once in call to " << callee;
          error(msg.str(), a->pstate());
        }
      ...
      else
      {
        ...
        {
          std::stringstream msg;
          msg << "argument " << a->name() << " of " << callee << "cannot be used as named argument";
          error(msg.str(), a->pstate());
        }
    ...
    for (size_t i = ip; i < LP; ++i)
    {
      ...
      if (!env->has_local(leftover->name()))
      {
        if (leftover->is_rest_parameter())
        {
          ...
          SASS_MEMORY_NEW(List, leftover->pstate(), 0, SASS_COMMA, true);
        }
