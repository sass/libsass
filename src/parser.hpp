...
namespace Sass {
  ...
  private:
    void add_single_file (Import* imp, std::string load_path);
    void import_single_file (Import* imp, std::string load_path);
  public:

    enum Syntactic_Context { nothing, mixin_def, function_def };
    bool do_import(const std::string& load_path, Import* imp, std::vector<Sass_Importer_Entry> importers, bool only_one = true);
