...
inline std::string pstate_source_position(AST_Node_Ptr node)
{
  ...
  str << (start.file == std::string::npos ? -1 : start.file) << "@[" << start.line << ":"
      << start.column << "]"
      << "-[" << end.line << ":" << end.column << "]";
#ifdef DEBUG_SHARED_PTR
  str << "x" << node->getRefCount() << ""
      << " " << node->getDbgFile() << "@" << node->getDbgLine();
#endif
  return str.str();
}
...
inline void debug_ast(AST_Node_Ptr node, std::string ind, Env* env)
{
  ...
  else if (Cast<Selector_List>(node))
  {
    ...
    for (const Complex_Selector_Obj& i : selector->elements())
    {
      debug_ast(i, ind + " ", env);
    }
    ...
    //    std::cerr << ind << "Expression " << expression << " " << expression->concrete_type() <<
    //    std::endl;
  }
  ...
  else if (Cast<Complex_Selector>(node))
  {
    ...
    std::cerr << ind << "Complex_Selector " << selector << " (" << pstate_source_position(node) << ")"
              << " <" << selector->hash() << ">"
  ...
  else if (Cast<Compound_Selector>(node))
  {
    ...
    std::cerr << (selector->has_line_feed() ? " [line-feed]" : " -");
    std::cerr << " <" << prettyprint(selector->pstate().token.ws_before()) << ">" << std::endl;
    for (const Simple_Selector_Obj& i : selector->elements())
    {
      debug_ast(i, ind + " ", env);
    }
  }
  ...
  else if (Cast<Placeholder_Selector>(node))
  {
    ...
    std::cerr << " (" << pstate_source_position(selector) << ")"
              ...
              << (selector->is_optional() ? " [is_optional]" : " -")
              << (selector->has_line_break() ? " [line-break]" : " -")
              << (selector->has_line_feed() ? " [line-feed]" : " -") << std::endl;
  }
  ...
  else if (Cast<Selector_Schema>(node))
  {
    ...
    std::cerr << " (" << pstate_source_position(node) << ")"
              << " [@media:" << selector->media_block() << "]"
              << (selector->connect_parent() ? " [connect-parent]" : " -") << std::endl;
  ...
  else if (Cast<Media_Query_Expression>(node))
  {
    ...
    std::cerr << " (" << pstate_source_position(node) << ")";
    std::cerr << (block->is_interpolated() ? " [is_interpolated]" : " -") << std::endl;
  ...
  else if (Cast<Media_Query>(node))
  {
    ...
    std::cerr << (block->is_negated() ? " [is_negated]" : " -")
              << (block->is_restricted() ? " [is_restricted]" : " -") << std::endl;
  ...
  else if (Cast<Media_Block>(node))
  {
    ...
    if (block->block())
      for (const Statement_Obj& i : block->block()->elements())
      {
        debug_ast(i, ind + " ", env);
      }
  }
  ...
  else if (Cast<Supports_Operator>(node))
  {
    ...
    std::cerr << " (" << pstate_source_position(node) << ")" << std::endl;
  ...
  else if (Cast<Block>(node))
  {
    ...
    for (const Statement_Obj& i : root_block->elements())
    {
      debug_ast(i, ind + " ", env);
    }
  }
  ...
  else if (Cast<Comment>(node))
  {
    ...
    std::cerr << " (" << pstate_source_position(node) << ")";
    std::cerr << " " << block->tabs() << " <" << prettyprint(block->pstate().token.ws_before())
              << ">" << std::endl;
  ...
  else if (Cast<Import>(node))
  {
    ...
    for (auto imp : block->urls())
      debug_ast(imp, ind + "@: ", env);
  ...
  else if (Cast<Keyframe_Rule>(node))
  {
    ...
    if (has_block->block())
      for (const Statement_Obj& i : has_block->block()->elements())
      {
        debug_ast(i, ind + " ", env);
      }
  }
  ...
  else if (Cast<Definition>(node))
  {
    ...
    std::cerr
    << " [type: " << (block->type() == Sass::Definition::Type::MIXIN ? "Mixin " : "Function ") << "] ";
  ...
  else if (Cast<Block>(node))
  {
    ...
    for (const Statement_Obj& i : block->elements())
    {
      debug_ast(i, ind + " ", env);
    }
  }
  ...
  else if (Cast<Arguments>(node))
  {
    ...
    std::cerr << std::endl;
    for (const Argument_Obj& i : expression->elements())
    {
      debug_ast(i, ind + " ", env);
    }
  }
  ...
  else if (Cast<Parameters>(node))
  {
    ...
    std::cerr << std::endl;
    for (const Parameter_Obj& i : expression->elements())
    {
      debug_ast(i, ind + " ", env);
    }
  }
  ...
  else if (Cast<Color>(node))
  {
    ...
    std::cerr << " [delayed: " << expression->is_delayed() << "] ";
    std::cerr << " [interpolant: " << expression->is_interpolant() << "] ";
    std::cerr << " [" << expression->r() << ":" << expression->g() << ":" << expression->b() << "@"
              << expression->a() << "]" << std::endl;
  }
  ...
  else if (Cast<Expression>(node))
  {
    ...
    {
    case Expression::Concrete_Type::NONE:
      std::cerr << " [NONE]";
      break;
    case Expression::Concrete_Type::BOOLEAN:
      std::cerr << " [BOOLEAN]";
      break;
    case Expression::Concrete_Type::NUMBER:
      std::cerr << " [NUMBER]";
      break;
    case Expression::Concrete_Type::COLOR:
      std::cerr << " [COLOR]";
      break;
    case Expression::Concrete_Type::STRING:
      std::cerr << " [STRING]";
      break;
    case Expression::Concrete_Type::LIST:
      std::cerr << " [LIST]";
      break;
    case Expression::Concrete_Type::MAP:
      std::cerr << " [MAP]";
      break;
    case Expression::Concrete_Type::SELECTOR:
      std::cerr << " [SELECTOR]";
      break;
    case Expression::Concrete_Type::NULL_VAL:
      std::cerr << " [NULL_VAL]";
      break;
    case Expression::Concrete_Type::C_WARNING:
      std::cerr << " [C_WARNING]";
      break;
    case Expression::Concrete_Type::C_ERROR:
      std::cerr << " [C_ERROR]";
      break;
    case Expression::Concrete_Type::FUNCTION:
      std::cerr << " [FUNCTION]";
      break;
    case Expression::Concrete_Type::NUM_TYPES:
      std::cerr << " [NUM_TYPES]";
      break;
    }
    std::cerr << std::endl;
