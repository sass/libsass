  ...
  namespace File
  {
...
#else
      struct stat st_buf;
      return (stat(path.c_str(), &st_buf) == 0) && (!S_ISDIR(st_buf.st_mode));
#endif
    }
    ...
    bool is_absolute_path(const std::string& path)
    {
      ...
      if (path[i] && Prelexer::is_alpha(path[i]))
      {
        ...
        while (path[i] && Prelexer::is_alnum(path[i]))
          ++i;
        i = i && path[i] == ':' ? i + 1 : 0;
      }
      return path[i] == '/';
    ...
    std::string make_canonical_path(std::string path)
    {
      ...
      while (path.length() > 1 && path.substr(0, 2) == "./")
        path.erase(0, 2);
      while ((pos = path.length()) > 1 && path.substr(pos - 2) == "/.")
        path.erase(pos - 2);
    ...
    // (4) given + extension
    std::vector<Include>
    resolve_includes(const std::string& root, const std::string& file, const std::vector<std::string>& exts)
    {
