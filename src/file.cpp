...
namespace Sass {
  namespace File {
    ...
    {
      return make_canonical_path(join_paths(cwd, path));
    }
    ...
    std::string find_file(const std::string& file, const std::vector<std::string> paths)
    {
      ...
      {
        std::vector<Sass_Include> resolved(resolve_file(paths[i], file));
