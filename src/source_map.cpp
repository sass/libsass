...
namespace Sass
{
  ...
  std::string SourceMap::serialize_mappings()
  {
    ...
    for (size_t i = 0; i < mappings.size(); ++i)
    {
      ...
      // generated column
      result +=
      base64vlq.encode(static_cast<int>(generated_column) - static_cast<int>(previous_generated_column));
      ...
      // source column
      result +=
      base64vlq.encode(static_cast<int>(original_column) - static_cast<int>(previous_original_column));
  ...
  ParserState SourceMap::remap(const ParserState& pstate)
  {
    for (size_t i = 0; i < mappings.size(); ++i)
    {
      if (mappings[i].generated_position.file == pstate.file &&
