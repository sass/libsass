...
namespace Sass
{
  ...
  class AST_Node : public SharedObj
  {
    ...
    AST_Node(const AST_Node* ptr) : pstate_(ptr->pstate_)
    {
    }
    ...
    {
      return to_string({ INSPECT, 5 });
    }
    virtual std::string to_sass() const
    {
      return to_string({ TO_SASS, 5 });
    }
  ...
  {
    return ptr && typeid(T) == typeid(*ptr) ? static_cast<T*>(ptr) : NULL;
  };

  template <class T> const T* Cast(const AST_Node* ptr)
  {
    return ptr && typeid(T) == typeid(*ptr) ? static_cast<const T*>(ptr) : NULL;
  };
  ...
  class Expression : public AST_Node
  {
    ...
    public:
    Expression(ParserState pstate, bool d = false, bool e = false, bool i = false, Concrete_Type ct = NONE)
    : AST_Node(pstate), is_delayed_(d), is_expanded_(e), is_interpolant_(i), concrete_type_(ct)
    {
    }
    Expression(const Expression* ptr)
    : AST_Node(ptr), is_delayed_(ptr->is_delayed_), is_expanded_(ptr->is_expanded_),
      is_interpolant_(ptr->is_interpolant_), concrete_type_(ptr->concrete_type_)
    {
    }
    ...
    virtual bool has_interpolant() const
    {
      return is_interpolant();
    }
    virtual bool is_left_interpolant() const
    {
      return is_interpolant();
    }
    virtual bool is_right_interpolant() const
    {
      return is_interpolant();
    }
    virtual std::string inspect() const
  ...
  {
    public:
    PreValue(ParserState pstate, bool d = false, bool e = false, bool i = false, Concrete_Type ct = NONE)
    : Expression(pstate, d, e, i, ct)
    {
    }
    PreValue(const PreValue* ptr) : Expression(ptr)
    {
    }
  ...
  {
    public:
    Value(ParserState pstate, bool d = false, bool e = false, bool i = false, Concrete_Type ct = NONE)
    : Expression(pstate, d, e, i, ct)
    {
    }
    Value(const Value* ptr) : Expression(ptr)
    {
    }
...
namespace std
{
  template <> struct hash<Sass::Expression_Obj>
  {
  ...
  template <> struct equal_to<Sass::Expression_Obj>
  {
...
namespace Sass
{
  ...
  /////////////////////////////////////////////////////////////////////////////
  template <typename T> class Vectorized
  {
    ...
    std::vector<T>& elements(std::vector<T>& e)
    {
      elements_ = e;
      return elements_;
    }
    ...
    typename std::vector<T>::iterator erase(typename std::vector<T>::iterator el)
    {
      return elements_.erase(el);
    }
    typename std::vector<T>::const_iterator erase(typename std::vector<T>::const_iterator el)
    {
      return elements_.erase(el);
    }
  };
  template <typename T> inline Vectorized<T>::~Vectorized()
  {
  }
  ...
  class Hashed
  {
    ...
    public:
    Hashed(size_t s = 0)
    : elements_(ExpressionMap(s)), list_(std::vector<Expression_Obj>()), hash_(0), duplicate_key_(NULL)
    {
      elements_.reserve(s);
      list_.reserve(s);
    }
    ...
    bool has(Expression_Obj k) const
    {
      return elements_.count(k) == 1;
    }
    Expression_Obj at(Expression_Obj k) const;
    bool has_duplicate_key() const
    {
      return duplicate_key_ != 0;
    }
    Expression_Obj get_duplicate_key() const
    {
      return duplicate_key_;
    }
    const ExpressionMap elements()
    {
      return elements_;
    }
    ...
    const ExpressionMap& pairs() const
    {
      return elements_;
    }
    const std::vector<Expression_Obj>& keys() const
    {
      return list_;
    }

    //    std::unordered_map<Expression_Obj, Expression_Obj>::iterator end() { return
    //    elements_.end(); }
    //    std::unordered_map<Expression_Obj, Expression_Obj>::iterator begin() { return
    //    elements_.begin(); }
    //    std::unordered_map<Expression_Obj, Expression_Obj>::const_iterator end() const { return
    //    elements_.end(); }
    //    std::unordered_map<Expression_Obj, Expression_Obj>::const_iterator begin() const { return
    //    elements_.begin(); }
  };
  ...
  class Statement : public AST_Node
  {
    ...
    : AST_Node(ptr), statement_type_(ptr->statement_type_), tabs_(ptr->tabs_), group_end_(ptr->group_end_)
    {
    }
    virtual ~Statement() = 0;
  ...
  class Block : public Statement, public Vectorized<Statement_Obj>
  {
    ...
    : Statement(pstate), Vectorized<Statement_Obj>(s), is_root_(r)
    {
    }
    Block(const Block* ptr)
    : Statement(ptr), Vectorized<Statement_Obj>(*ptr), is_root_(ptr->is_root_)
    {
    }
    virtual bool has_content()
    {
  ...
  class Has_Block : public Statement
  {
    ...
    public:
    Has_Block(ParserState pstate, Block_Obj b) : Statement(pstate), block_(b)
    {
    }
    Has_Block(const Has_Block* ptr) : Statement(ptr), block_(ptr->block_)
    {
    }
    virtual bool has_content()
    {
  ...
  class Ruleset : public Has_Block
  {
    ...
    Ruleset(const Ruleset* ptr) : Has_Block(ptr), selector_(ptr->selector_), is_root_(ptr->is_root_)
    {
      statement_type(RULESET);
    }
  ...
  class Bubble : public Statement
  {
    ...
    Bubble(const Bubble* ptr) : Statement(ptr), node_(ptr->node_), group_end_(ptr->group_end_)
    {
    }
    bool bubbles()
    {
  ...
  class Trace : public Has_Block
  {
    ...
    public:
    Trace(ParserState pstate, std::string n, Block_Obj b = 0) : Has_Block(pstate, b), name_(n)
    {
    }
    Trace(const Trace* ptr) : Has_Block(ptr), name_(ptr->name_)
    {
    }
  ...
  class Media_Block : public Has_Block
  {
    ...
    Media_Block(const Media_Block* ptr) : Has_Block(ptr), media_queries_(ptr->media_queries_)
    {
      statement_type(MEDIA);
    }
    bool bubbles()
  ...
  class Directive : public Has_Block
  {
    ...
    : Has_Block(pstate, b), keyword_(kwd), selector_(sel),
      value_(val) // set value manually if needed
    {
      statement_type(DIRECTIVE);
    }
    Directive(const Directive* ptr)
    : Has_Block(ptr), keyword_(ptr->keyword_), selector_(ptr->selector_),
    ...
    bool is_media()
    {
      return keyword_.compare("@-webkit-media") == 0 || keyword_.compare("@-moz-media") == 0 ||
             keyword_.compare("@-o-media") == 0 || keyword_.compare("@media") == 0;
    }
    bool is_keyframes()
    {
      return keyword_.compare("@-webkit-keyframes") == 0 || keyword_.compare("@-moz-keyframes") == 0 ||
             keyword_.compare("@-o-keyframes") == 0 || keyword_.compare("@keyframes") == 0;
    }
  ...
  class Keyframe_Rule : public Has_Block
  {
    ...
    public:
    Keyframe_Rule(ParserState pstate, Block_Obj b) : Has_Block(pstate, b), name_()
    {
      statement_type(KEYFRAMERULE);
    }
    Keyframe_Rule(const Keyframe_Rule* ptr) : Has_Block(ptr), name_(ptr->name_)
    {
      statement_type(KEYFRAMERULE);
    }
  ...
  class Declaration : public Has_Block
  {
    ...
    public:
    Declaration(ParserState pstate, String_Obj prop, Expression_Obj val, bool i = false, Block_Obj b = 0)
    : Has_Block(pstate, b), property_(prop), value_(val), is_important_(i), is_indented_(false)
    {
      statement_type(DECLARATION);
    }
    Declaration(const Declaration* ptr)
    : Has_Block(ptr), property_(ptr->property_), value_(ptr->value_),
      is_important_(ptr->is_important_), is_indented_(ptr->is_indented_)
    {
      statement_type(DECLARATION);
    }
  ...
  class Assignment : public Statement
  {
    ...
    : Statement(ptr), variable_(ptr->variable_), value_(ptr->value_), is_default_(ptr->is_default_),
  ...
  class Import : public Statement
  {
    ...
    : Statement(pstate), urls_(std::vector<Expression_Obj>()), incs_(std::vector<Include>()),
      import_queries_()
    {
      statement_type(IMPORT);
    }
    Import(const Import* ptr)
    : Statement(ptr), urls_(ptr->urls_), incs_(ptr->incs_), import_queries_(ptr->import_queries_)
    {
      statement_type(IMPORT);
    }
  ...
  class Import_Stub : public Statement
  {
    ...
    std::string abs_path()
    {
      return resource_.abs_path;
    };
    std::string imp_path()
    {
      return resource_.imp_path;
    };
    Include resource()
    {
      return resource_;
    ...
    Import_Stub(ParserState pstate, Include res) : Statement(pstate), resource_(res)
    {
      statement_type(IMPORT_STUB);
    }
    Import_Stub(const Import_Stub* ptr) : Statement(ptr), resource_(ptr->resource_)
    {
      statement_type(IMPORT_STUB);
    }
  ...
  class Warning : public Statement
  {
    ...
    public:
    Warning(ParserState pstate, Expression_Obj msg) : Statement(pstate), message_(msg)
    {
      statement_type(WARNING);
    }
    Warning(const Warning* ptr) : Statement(ptr), message_(ptr->message_)
    {
      statement_type(WARNING);
    }
  ...
  class Error : public Statement
  {
    ADD_PROPERTY(Expression_Obj, message)
    public:
    Error(ParserState pstate, Expression_Obj msg) : Statement(pstate), message_(msg)
    {
      statement_type(ERROR);
    }
    Error(const Error* ptr) : Statement(ptr), message_(ptr->message_)
    {
      statement_type(ERROR);
    }
  ...
  class Debug : public Statement
  {
    ADD_PROPERTY(Expression_Obj, value)
    public:
    Debug(ParserState pstate, Expression_Obj val) : Statement(pstate), value_(val)
    {
      statement_type(DEBUGSTMT);
    }
    Debug(const Debug* ptr) : Statement(ptr), value_(ptr->value_)
    {
      statement_type(DEBUGSTMT);
    }
  ...
  class Comment : public Statement
  {
    ...
    : Statement(ptr), text_(ptr->text_), is_important_(ptr->is_important_)
    {
      statement_type(COMMENT);
    }
    virtual bool is_invisible() const
  ...
  class If : public Has_Block
  {
    ...
    If(const If* ptr) : Has_Block(ptr), predicate_(ptr->predicate_), alternative_(ptr->alternative_)
    {
      statement_type(IF);
    }
    virtual bool has_content()
  ...
  class For : public Has_Block
  {
    ...
    : Has_Block(pstate, b), variable_(var), lower_bound_(lo), upper_bound_(hi), is_inclusive_(inc)
    {
      statement_type(FOR);
    }
    For(const For* ptr)
    : Has_Block(ptr), variable_(ptr->variable_), lower_bound_(ptr->lower_bound_),
      upper_bound_(ptr->upper_bound_), is_inclusive_(ptr->is_inclusive_)
    {
      statement_type(FOR);
    }
  ...
  class Each : public Has_Block
  {
    ...
    Each(const Each* ptr) : Has_Block(ptr), variables_(ptr->variables_), list_(ptr->list_)
    {
      statement_type(EACH);
    }
  ...
  class While : public Has_Block
  {
    ...
    While(const While* ptr) : Has_Block(ptr), predicate_(ptr->predicate_)
    {
      statement_type(WHILE);
    }
  ...
  class Return : public Statement
  {
    ADD_PROPERTY(Expression_Obj, value)
    public:
    Return(ParserState pstate, Expression_Obj val) : Statement(pstate), value_(val)
    {
      statement_type(RETURN);
    }
    Return(const Return* ptr) : Statement(ptr), value_(ptr->value_)
    {
      statement_type(RETURN);
    }
  ...
  class Extension : public Statement
  {
    ADD_PROPERTY(Selector_List_Obj, selector)
    public:
    Extension(ParserState pstate, Selector_List_Obj s) : Statement(pstate), selector_(s)
    {
      statement_type(EXTEND);
    }
    Extension(const Extension* ptr) : Statement(ptr), selector_(ptr->selector_)
    {
      statement_type(EXTEND);
    }
  ...
  class Definition : public Has_Block
  {
    ...
    : Has_Block(ptr), name_(ptr->name_), parameters_(ptr->parameters_), environment_(ptr->environment_),
      type_(ptr->type_), native_function_(ptr->native_function_), c_function_(ptr->c_function_),
      cookie_(ptr->cookie_), is_overload_stub_(ptr->is_overload_stub_), signature_(ptr->signature_)
    {
    }

    Definition(ParserState pstate, std::string n, Parameters_Obj params, Block_Obj b, Type t)
    : Has_Block(pstate, b), name_(n), parameters_(params), environment_(0), type_(t),
      native_function_(0), c_function_(0), cookie_(0), is_overload_stub_(false), signature_(0)
    {
    }
    Definition(ParserState pstate, Signature sig, std::string n, Parameters_Obj params, Native_Function func_ptr, bool overload_stub = false)
    : Has_Block(pstate, 0), name_(n), parameters_(params), environment_(0), type_(FUNCTION),
      native_function_(func_ptr), c_function_(0), cookie_(0), is_overload_stub_(overload_stub),
      signature_(sig)
    {
    }
    Definition(ParserState pstate, Signature sig, std::string n, Parameters_Obj params, Sass_Function_Entry c_func, bool whatever, bool whatever2)
    : Has_Block(pstate, 0), name_(n), parameters_(params), environment_(0), type_(FUNCTION),
      native_function_(0), c_function_(c_func), cookie_(sass_function_get_cookie(c_func)),
      is_overload_stub_(false), signature_(sig)
    {
    }
  ...
  class Mixin_Call : public Has_Block
  {
    ...
    : Has_Block(ptr), name_(ptr->name_), arguments_(ptr->arguments_)
    {
    }
  ...
  class Content : public Statement
  {
    ...
    public:
    Content(ParserState pstate) : Statement(pstate), media_block_(NULL)
    {
      statement_type(CONTENT);
    }
    Content(const Content* ptr) : Statement(ptr), media_block_(ptr->media_block_)
    {
      statement_type(CONTENT);
    }
  ...
  {
    void adjust_after_pushing(Expression_Obj e)
    {
      is_expanded(false);
    }
    ...
    : Value(pstate), Vectorized<Expression_Obj>(size), separator_(sep), is_arglist_(argl),
      is_bracketed_(bracket), from_selector_(false)
    {
      concrete_type(LIST);
    }
    List(const List* ptr)
    : Value(ptr), Vectorized<Expression_Obj>(*ptr), separator_(ptr->separator_),
      is_arglist_(ptr->is_arglist_), is_bracketed_(ptr->is_bracketed_), from_selector_(ptr->from_selector_)
    {
      concrete_type(LIST);
    }
    std::string type() const
    {
      return is_arglist_ ? "arglist" : "list";
    }
    static std::string type_name()
    {
      return "list";
    ...
    {
      return separator() == SASS_SPACE ? " " : (compressed ? "," : ", ");
    }
    bool is_invisible() const
    {
      return empty() && !is_bracketed();
    }
  ...
  {
    void adjust_after_pushing(std::pair<Expression_Obj, Expression_Obj> p)
    {
      is_expanded(false);
    }

    public:
    Map(ParserState pstate, size_t size = 0) : Value(pstate), Hashed(size)
    {
      concrete_type(MAP);
    }
    Map(const Map* ptr) : Value(ptr), Hashed(*ptr)
    {
      concrete_type(MAP);
    }
    std::string type() const
  ...
  class Binary_Expression : public PreValue
  {
    ...
    public:
    Binary_Expression(ParserState pstate, Operand op, Expression_Obj lhs, Expression_Obj rhs)
    : PreValue(pstate), op_(op), left_(lhs), right_(rhs), hash_(0)
    {
    }
    Binary_Expression(const Binary_Expression* ptr)
    : PreValue(ptr), op_(ptr->op_), left_(ptr->left_), right_(ptr->right_), hash_(ptr->hash_)
    {
    }
    ...
    virtual bool operator==(const Expression& rhs) const
    {
      try
      {
        ...
        if (m == 0) return false;
        return type() == m->type() && *left() == *m->left() && *right() == *m->right();
      }
      catch (std::bad_cast&)
      {
        return false;
  ...
  class Unary_Expression : public Expression
  {
    ...
    : Expression(ptr), optype_(ptr->optype_), operand_(ptr->operand_), hash_(ptr->hash_)
    {
    }
    const std::string type_name()
    {
    ...
    virtual bool operator==(const Expression& rhs) const
    {
      try
      {
        ...
        if (m == 0) return false;
        return type() == m->type() && *operand() == *m->operand();
      }
      catch (std::bad_cast&)
      {
        return false;
  ...
  class Argument : public Expression
  {
    ...
    : Expression(pstate), value_(val), name_(n), is_rest_argument_(rest),
      is_keyword_argument_(keyword), hash_(0)
    {
    ...
    : Expression(ptr), value_(ptr->value_), name_(ptr->name_), is_rest_argument_(ptr->is_rest_argument_),
      is_keyword_argument_(ptr->is_keyword_argument_), hash_(ptr->hash_)
    {
      if (!name_.empty() && is_rest_argument_)
      {
        error("variable-length argument may not be passed by name", pstate_);
  ...
  class Arguments : public Expression, public Vectorized<Argument_Obj>
  {
    ...
    : Expression(pstate), Vectorized<Argument_Obj>(), has_named_arguments_(false),
      has_rest_argument_(false), has_keyword_argument_(false)
    {
    }
    Arguments(const Arguments* ptr)
    : Expression(ptr), Vectorized<Argument_Obj>(*ptr), has_named_arguments_(ptr->has_named_arguments_),
      has_rest_argument_(ptr->has_rest_argument_), has_keyword_argument_(ptr->has_keyword_argument_)
    {
    }
  ...
  class Function_Call : public PreValue
  {
    ...
    : PreValue(ptr), name_(ptr->name_), arguments_(ptr->arguments_), via_call_(ptr->via_call_),
      cookie_(ptr->cookie_), hash_(ptr->hash_)
    {
      concrete_type(FUNCTION);
    }
  ...
  class Function_Call_Schema : public Expression
  {
    ...
    : Expression(ptr), name_(ptr->name_), arguments_(ptr->arguments_)
    {
      concrete_type(STRING);
    }
  ...
  class Variable : public PreValue
  {
    ADD_CONSTREF(std::string, name)
    public:
    Variable(ParserState pstate, std::string n) : PreValue(pstate), name_(n)
    {
      concrete_type(VARIABLE);
    }
    Variable(const Variable* ptr) : PreValue(ptr), name_(ptr->name_)
    {
      concrete_type(VARIABLE);
    }
  ...
  class Number : public Value
  {
    ...
    : Value(ptr), value_(ptr->value_), zero_(ptr->zero_), numerator_units_(ptr->numerator_units_),
      denominator_units_(ptr->denominator_units_), hash_(ptr->hash_)
    {
    ...
    std::vector<std::string>& numerator_units()
    {
      return numerator_units_;
    }
    std::vector<std::string>& denominator_units()
    {
      return denominator_units_;
    }
    const std::vector<std::string>& numerator_units() const
    {
      return numerator_units_;
    }
    const std::vector<std::string>& denominator_units() const
    {
      return denominator_units_;
    }
    std::string type() const
  ...
  class Color : public Value
  {
    ...
    : Value(pstate), r_(r), g_(g), b_(b), a_(a), disp_(disp), hash_(0)
    {
      concrete_type(COLOR);
    }
    Color(const Color* ptr)
    : Value(ptr), r_(ptr->r_), g_(ptr->g_), b_(ptr->b_), a_(ptr->a_), disp_(ptr->disp_), hash_(ptr->hash_)
    {
      concrete_type(COLOR);
    }
    std::string type() const
  ...
  class Custom_Error : public Value
  {
    ...
    public:
    Custom_Error(ParserState pstate, std::string msg) : Value(pstate), message_(msg)
    {
      concrete_type(C_ERROR);
    }
    Custom_Error(const Custom_Error* ptr) : Value(ptr), message_(ptr->message_)
    {
      concrete_type(C_ERROR);
    }
    virtual bool operator==(const Expression& rhs) const;
  ...
  class Custom_Warning : public Value
  {
    ADD_CONSTREF(std::string, message)
    public:
    Custom_Warning(ParserState pstate, std::string msg) : Value(pstate), message_(msg)
    {
      concrete_type(C_WARNING);
    }
    Custom_Warning(const Custom_Warning* ptr) : Value(ptr), message_(ptr->message_)
    {
      concrete_type(C_WARNING);
    }
    virtual bool operator==(const Expression& rhs) const;
  ...
  class Boolean : public Value
  {
    ...
    public:
    Boolean(ParserState pstate, bool val) : Value(pstate), value_(val), hash_(0)
    {
      concrete_type(BOOLEAN);
    }
    Boolean(const Boolean* ptr) : Value(ptr), value_(ptr->value_), hash_(ptr->hash_)
    {
      concrete_type(BOOLEAN);
    }
    virtual operator bool()
  ...
  {
    public:
    String(ParserState pstate, bool delayed = false) : Value(pstate, delayed)
    {
      concrete_type(STRING);
    }
    String(const String* ptr) : Value(ptr)
    {
      concrete_type(STRING);
    }
    static std::string type_name()
  ...
  class String_Schema : public String, public Vectorized<Expression_Obj>
  {
    ...
    : String(ptr), Vectorized<Expression_Obj>(*ptr), hash_(ptr->hash_)
    {
      concrete_type(STRING);
    }
  ...
  class String_Constant : public String
  {
    ...
    : String(ptr), quote_mark_(ptr->quote_mark_),
      can_compress_whitespace_(ptr->can_compress_whitespace_), value_(ptr->value_), hash_(ptr->hash_)
    {
    }
    String_Constant(ParserState pstate, std::string val)
    : String(pstate), quote_mark_(0), can_compress_whitespace_(false), value_(read_css_string(val)),
      hash_(0)
    {
    }
    String_Constant(ParserState pstate, const char* beg)
    : String(pstate), quote_mark_(0), can_compress_whitespace_(false),
      value_(read_css_string(std::string(beg))), hash_(0)
    {
    }
    String_Constant(ParserState pstate, const char* beg, const char* end)
    : String(pstate), quote_mark_(0), can_compress_whitespace_(false),
      value_(read_css_string(std::string(beg, end - beg))), hash_(0)
    {
    }
    String_Constant(ParserState pstate, const Token& tok)
    : String(pstate), quote_mark_(0), can_compress_whitespace_(false),
      value_(read_css_string(std::string(tok.begin, tok.end))), hash_(0)
    {
    }
    std::string type() const
    {
  ...
  {
    public:
    String_Quoted(ParserState pstate,
                  std::string val,
                  char q = 0,
                  bool keep_utf8_escapes = false,
                  bool skip_unquoting = false,
    ...
    String_Quoted(const String_Quoted* ptr) : String_Constant(ptr)
    {
    }
    virtual bool operator==(const Expression& rhs) const;
    virtual std::string inspect() const; // quotes are forced on inspection
  ...
  // Media queries.
  /////////////////
  class Media_Query : public Expression, public Vectorized<Media_Query_Expression_Obj>
  {
    ...
    : Expression(pstate), Vectorized<Media_Query_Expression_Obj>(s), media_type_(t), is_negated_(n),
      is_restricted_(r)
    {
    }
    Media_Query(const Media_Query* ptr)
    : Expression(ptr), Vectorized<Media_Query_Expression_Obj>(*ptr), media_type_(ptr->media_type_),
  ...
  class Media_Query_Expression : public Expression
  {
    ...
    : Expression(ptr), feature_(ptr->feature_), value_(ptr->value_), is_interpolated_(ptr->is_interpolated_)
    {
    }
  ...
  class Supports_Block : public Has_Block
  {
    ...
    Supports_Block(const Supports_Block* ptr) : Has_Block(ptr), condition_(ptr->condition_)
    {
      statement_type(SUPPORTS);
    }
    bool bubbles()
  ...
  {
    public:
    Supports_Condition(ParserState pstate) : Expression(pstate)
    {
    }
    Supports_Condition(const Supports_Condition* ptr) : Expression(ptr)
    {
    }
  ...
  class Supports_Operator : public Supports_Condition
  {
    ...
    : Supports_Condition(ptr), left_(ptr->left_), right_(ptr->right_), operand_(ptr->operand_)
    {
    }
    virtual bool needs_parens(Supports_Condition_Obj cond) const;
  ...
  class Supports_Declaration : public Supports_Condition
  {
    ...
    : Supports_Condition(ptr), feature_(ptr->feature_), value_(ptr->value_)
    {
    }
    virtual bool needs_parens(Supports_Condition_Obj cond) const
    {
  ...
  class Supports_Interpolation : public Supports_Condition
  {
    ...
    : Supports_Condition(ptr), value_(ptr->value_)
    {
    }
    virtual bool needs_parens(Supports_Condition_Obj cond) const
    {
  ...
  class At_Root_Query : public Expression
  {
    ...
    : Expression(ptr), feature_(ptr->feature_), value_(ptr->value_)
    {
    }
  ...
  class At_Root_Block : public Has_Block
  {
    ...
    At_Root_Block(const At_Root_Block* ptr) : Has_Block(ptr), expression_(ptr->expression_)
    {
      statement_type(ATROOT);
    }
    bool bubbles()
  ...
  {
    public:
    Null(ParserState pstate) : Value(pstate)
    {
      concrete_type(NULL_VAL);
    }
    Null(const Null* ptr) : Value(ptr)
    {
      concrete_type(NULL_VAL);
    }
    std::string type() const
  ...
  class Parameters : public AST_Node, public Vectorized<Parameter_Obj>
  {
    ...
    : AST_Node(pstate), Vectorized<Parameter_Obj>(), has_optional_parameters_(false),
      has_rest_parameter_(false)
    {
    }
    Parameters(const Parameters* ptr)
    : AST_Node(ptr), Vectorized<Parameter_Obj>(*ptr),
      has_optional_parameters_(ptr->has_optional_parameters_), has_rest_parameter_(ptr->has_rest_parameter_)
    {
    }
  ...
  class Selector : public Expression
  {
    ...
    : Expression(pstate), has_line_feed_(false), has_line_break_(false), is_optional_(false),
      media_block_(0), hash_(0)
    {
    ...
    : Expression(ptr),
      ...
      has_line_feed_(ptr->has_line_feed_), has_line_break_(ptr->has_line_break_),
      is_optional_(ptr->is_optional_), media_block_(ptr->media_block_), hash_(ptr->hash_)
    {
      concrete_type(SELECTOR);
    }
    virtual ~Selector() = 0;
  ...
  class Selector_Schema : public AST_Node
  {
    ...
    : AST_Node(pstate), contents_(c), connect_parent_(true), media_block_(NULL), hash_(0)
    {
    }
    Selector_Schema(const Selector_Schema* ptr)
    : AST_Node(ptr), contents_(ptr->contents_), connect_parent_(ptr->connect_parent_),
      media_block_(ptr->media_block_), hash_(ptr->hash_)
    {
    }
    virtual bool has_parent_ref() const;
    virtual bool has_real_parent_ref() const;
  ...
  class Simple_Selector : public Selector
  {
    ...
    : Selector(ptr), ns_(ptr->ns_), name_(ptr->name_), has_ns_(ptr->has_ns_)
    {
      simple_type(SIMPLE);
    }
    ...
    inline bool operator!=(const Simple_Selector& rhs) const
    {
      return !(*this == rhs);
    }
  ...
  class Parent_Selector : public Simple_Selector
  {
    ...
    public:
    Parent_Selector(ParserState pstate, bool r = true) : Simple_Selector(pstate, "&"), real_(r)
    { /* has_reference(true); */
    }
    Parent_Selector(const Parent_Selector* ptr) : Simple_Selector(ptr), real_(ptr->real_)
    { /* has_reference(true); */
    }
    bool is_real_parent_ref() const
    {
      return real();
    };
    virtual bool has_parent_ref() const
  ...
  {
    public:
    Placeholder_Selector(ParserState pstate, std::string n) : Simple_Selector(pstate, n)
    {
    }
    Placeholder_Selector(const Placeholder_Selector* ptr) : Simple_Selector(ptr)
    {
    }
    virtual unsigned long specificity() const
    {
  ...
  {
    public:
    Element_Selector(ParserState pstate, std::string n) : Simple_Selector(pstate, n)
    {
    }
    Element_Selector(const Element_Selector* ptr) : Simple_Selector(ptr)
    {
    }
    virtual unsigned long specificity() const
    {
  ...
  {
    public:
    Class_Selector(ParserState pstate, std::string n) : Simple_Selector(pstate, n)
    {
    }
    Class_Selector(const Class_Selector* ptr) : Simple_Selector(ptr)
    {
    }
    virtual unsigned long specificity() const
    {
  ...
  {
    public:
    Id_Selector(ParserState pstate, std::string n) : Simple_Selector(pstate, n)
    {
    }
    Id_Selector(const Id_Selector* ptr) : Simple_Selector(ptr)
    {
    }
    virtual unsigned long specificity() const
    {
  ...
  class Attribute_Selector : public Simple_Selector
  {
    ...
    : Simple_Selector(ptr), matcher_(ptr->matcher_), value_(ptr->value_)
    {
      simple_type(ATTR_SEL);
    }
    virtual size_t hash()
  ...
  class Pseudo_Selector : public Simple_Selector
  {
    ...
    virtual unsigned long specificity() const
    {
      if (is_pseudo_element()) return Constants::Specificity_Element;
  ...
  /////////////////////////////////////////////////
  // Wrapped selector -- pseudo selector that takes a list of selectors as argument(s) e.g.,
  // :not(:first-of-type), :-moz-any(ol p.blah, ul, menu, dir)
  /////////////////////////////////////////////////
  class Wrapped_Selector : public Simple_Selector
  {
    ...
    Wrapped_Selector(const Wrapped_Selector* ptr) : Simple_Selector(ptr), selector_(ptr->selector_)
    {
      simple_type(WRAPPED_SEL);
    }
  ...
  class Compound_Selector : public Selector, public Vectorized<Simple_Selector_Obj>
  {
    ...
    : Selector(pstate), Vectorized<Simple_Selector_Obj>(s), extended_(false), has_parent_reference_(false)
    {
    }
    Compound_Selector(const Compound_Selector* ptr)
    : Selector(ptr), Vectorized<Simple_Selector_Obj>(*ptr), extended_(ptr->extended_),
    ...
    Simple_Selector_Ptr base() const
    {
      ...
      if (Cast<Element_Selector>((*this)[0])) return (*this)[0];
      return 0;
    }
    ...
    {
      return length() == 1 && Cast<Parent_Selector>((*this)[0]);
    }
    ...
    virtual bool operator==(const Compound_Selector& rhs) const;
    inline bool operator!=(const Compound_Selector& rhs) const
    {
      return !(*this == rhs);
    }

    ComplexSelectorSet& sources()
    {
      return sources_;
    }
    void clearSources()
  ...
  class Complex_Selector : public Selector
  {
    ...
    : Selector(ptr), combinator_(ptr->combinator_), head_(ptr->head_), tail_(ptr->tail_),
    ...
    {
      return (!head() || head()->length() == 0) && combinator() == Combinator::ANCESTOR_OF;
    }
    ...
    Complex_Selector_Obj innermost()
    {
      return last();
    };
    ...
    virtual bool operator==(const Complex_Selector& rhs) const;
    inline bool operator!=(const Complex_Selector& rhs) const
    {
      return !(*this == rhs);
    }
  ...
  class Selector_List : public Selector, public Vectorized<Complex_Selector_Obj>
  {
    ...
    : Selector(pstate), Vectorized<Complex_Selector_Obj>(s), schema_(NULL), wspace_(0)
    {
    }
    Selector_List(const Selector_List* ptr)
    : Selector(ptr), Vectorized<Complex_Selector_Obj>(*ptr), schema_(ptr->schema_), wspace_(ptr->wspace_)
    {
    }
    std::string type() const
    {
  ...
  struct cmp_complex_selector
  {
    inline bool operator()(const Complex_Selector_Obj l, const Complex_Selector_Obj r)
    {
      return (*l < *r);
    }
  };
  struct cmp_compound_selector
  {
    inline bool operator()(const Compound_Selector_Obj l, const Compound_Selector_Obj r)
    {
      return (*l < *r);
    }
  };
  struct cmp_simple_selector
  {
    inline bool operator()(const Simple_Selector_Obj l, const Simple_Selector_Obj r)
    {
      return (*l < *r);
    }
  };
