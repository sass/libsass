...
namespace Sass
{
  ...
  Inspect::Inspect(const Emitter& emi) : Emitter(emi)
  {
  }
  Inspect::~Inspect()
  {
  ...
  void Inspect::operator()(Declaration_Ptr dec)
  {
    ...
    append_indentation();
    if (dec->property()) dec->property()->perform(this);
  ...
  void Inspect::operator()(List_Ptr list)
  {
    ...
    if ((output_style() != COMPRESSED) && sep == ",")
      sep += " ";
    else if (in_media_block && sep != " ")
      sep += " "; // verified
    ...
    else if (output_style() == TO_SASS && list->length() == 1 && !list->from_selector() &&
             !Cast<List>(list->at(0)) && !Cast<Selector_List>(list->at(0)))
    {
      append_string(lbracket(list));
    }
    else if (!in_declaration && (list->separator() == SASS_HASH || (list->separator() == SASS_SPACE && in_space_array) ||
    ...
    if (list->separator() == SASS_SPACE)
      in_space_array = true;
    else if (list->separator() == SASS_COMMA)
      in_comma_array = true;
  ...
  void Inspect::operator()(Binary_Expression_Ptr expr)
  {
    ...
    if (in_media_block || (output_style() == INSPECT) ||
        (expr->op().ws_before && (!expr->is_interpolant()) &&
         (expr->is_left_interpolant() || expr->is_right_interpolant())
    ...
    if (in_media_block || (output_style() == INSPECT) ||
        (expr->op().ws_after && (!expr->is_interpolant()) &&
         (expr->is_left_interpolant() || expr->is_right_interpolant())))
      append_string(" ");
  ...
  {
    if (expr->optype() == Unary_Expression::PLUS)
      append_string("+");
    else
      append_string("-");
  ...
  void Inspect::operator()(Number_Ptr n)
  {
    ...
    bool is_int = pos_point == pos_fract || pos_point == std::string::npos;
    ...
    else
    {
      ...
      {
        bool at_dec_point = res[pos] == '.' || res[pos] == ',';
  ...
  void Inspect::operator()(Color_Ptr c)
  {
    ...
    else
    {
      ...
      if (color_to_name(numval)) res_name = color_to_name(numval);
    }
  ...
  void Inspect::operator()(Selector_List_Ptr g)
  {
    ...
    if (output_style() == TO_SASS && g->length() == 1 &&
        (!Cast<List>((*g)[0]) && !Cast<Selector_List>((*g)[0])))
    {
      append_string("(");
    }
