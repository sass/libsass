...
#define KEEP_COMMENT(converter) \
  ((converter.options & SASS2SCSS_KEEP_COMMENT) == SASS2SCSS_KEEP_COMMENT)
#define STRIP_COMMENT(converter) \
  ((converter.options & SASS2SCSS_STRIP_COMMENT) == SASS2SCSS_STRIP_COMMENT)
#define CONVERT_COMMENT(converter) \
  ((converter.options & SASS2SCSS_CONVERT_COMMENT) == SASS2SCSS_CONVERT_COMMENT)
...
#define IS_CSS_COMMENT(converter) \
  (converter.comment == "/*" || (converter.comment == "//" && CONVERT_COMMENT(converter)))

  // pretty printer helper function
  static std::string closer(const converter& converter)
  {
    return PRETTIFY(converter) == 0 ? " }" : PRETTIFY(converter) <= 1 ? " }" : "\n" + INDENT(converter) + "}";
  }

  // pretty printer helper function
  static std::string opener(const converter& converter)
  {
    return PRETTIFY(converter) == 0 ? " { " : PRETTIFY(converter) <= 2 ? " {" : "\n" + INDENT(converter) + "{";
  }
  ...
  static std::string removeMultilineComment(std::string& sass)
  {
    ...
    if (comment)
      clean += sass.substr(open_pos);
    else
      clean += sass.substr(close_pos);
  ...
  std::string flush(std::string& sass, converter& converter)
  {
    ...
    scss += PRETTIFY(converter) > 0 ? converter.whitespace : "";
  ...
  std::string process(std::string& sass, converter& converter)
  {
    ...
    else
    {
      ...
      // terminate some statements immediately
               ...
               sass.substr(pos_left, 6) == "@error" || sass.substr(pos_left, 8) == "@charset" ||
               sass.substr(pos_left, 10) == "@namespace")
      {
        sass = indent + sass.substr(pos_left);
      }
      // replace some specific sass shorthand directives (if not fallowed by a white space
      // character)
      ...
      else if (sass.substr(pos_left, 7) != "@return" && sass.substr(pos_left, 7) != "@extend" &&
               sass.substr(pos_left, 8) != "@include" && sass.substr(pos_left, 8) != "@content")
      {
