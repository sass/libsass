...
namespace Sass
{
  ...
  : wbuf(), opt(opt), indentation(0), scheduled_space(0), scheduled_linefeed(0),
    scheduled_delimiter(false), scheduled_mapping(0), in_comment(false), in_wrapped(false),
    in_media_block(false), in_declaration(false), in_space_array(false), in_comma_array(false)
  {
  }
  ...
  void Emitter::finalize(bool final)
  {
    ...
    if (scheduled_linefeed) scheduled_linefeed = 1;
    flush_schedules();
  }
  ...
  void Emitter::append_indentation()
  {
    ...
    if (scheduled_linefeed && indentation) scheduled_linefeed = 1;
    std::string indent = "";
  ...
  void Emitter::append_scope_closer(AST_Node_Ptr node)
  {
    --indentation;
    scheduled_linefeed = 0;
    if (output_style() == COMPRESSED) scheduled_delimiter = false;
    ...
    if (output_style() != COMPRESSED) scheduled_linefeed = 2;
  }
}
