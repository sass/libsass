...
/*
 ...
 - The print* functions print to cerr. This allows our testing frameworks (like sass-spec) to ignore
 the output, which
   is very helpful when debugging. The format of the output is mainly to wrap things in square
 brackets to match what
   ruby already outputs (to make comparisons easier).

 - For the direct porting effort, we're trying to port method-for-method until we get all the tests
 passing.
   Where applicable, I've tried to include the ruby code above the function for reference until all
 our tests pass.
   The ruby code isn't always directly portable, so I've tried to include any modified ruby code
 that was actually
   used for the porting.

 - DO NOT try to optimize yet. We get a tremendous benefit out of comparing the output of each stage
 of the extend to the ruby
   output at the same stage. This makes it much easier to determine where problems are. Try to keep
 as close to
   the ruby code as you can until we have all the sass-spec tests passing. Then, we should optimize.
 However, if you see
   something that could probably be optimized, let's not forget it. Add a // TODO: or //
 IMPROVEMENT: comment.
 ...
 - You may see a lot of comments that say "// TODO: is this the correct combinator?". See the
 comment referring to combinators
   in extendCompoundSelector for a more extensive explanation of my confusion. I think our
 divergence in data model from ruby
   sass causes this to be necessary.


 GLOBAL TODOS:

 - wrap the contents of the print functions in DEBUG preprocesser conditionals so they will be
 optimized away in non-debug mode.

 - consider making the extend* functions member functions to avoid passing around ctx and subset_map
 map around. This has the
   drawback that the implementation details of the operator are then exposed to the outside world,
 which is not ideal and
 ...
 - fix compilation warnings for unused Extend members if we really don't need those references
 anymore.
 */
...
namespace Sass
{
  ...
  // Print a string representation of a Compound_Selector
  static void
  printCompoundSelector(Compound_Selector_Ptr pCompoundSelector, const char* message = NULL, bool newline = true)
  {
  ...
  static void printSourcesSet(ComplexSelectorSet& sources, const char* message = NULL, bool newline = true)
  {
    ...
    // Convert to a deque of strings so we can sort since order doesn't matter in a set. This should
    // cut down on
    // the differences we see when debug printing.
    typedef std::deque<std::string> SourceStrings;
    SourceStrings sourceStrings;
    for (ComplexSelectorSet::iterator iterator = sources.begin(), iteratorEnd = sources.end();
    ...
    for (SourceStrings::iterator iterator = sourceStrings.begin(), iteratorEnd = sourceStrings.end();
         iterator != iteratorEnd; ++iterator)
    {
  ...
  std::ostream& operator<<(std::ostream& os, SubSetMapPairs& entries)
  {
    ...
    for (SubSetMapPairs::iterator iterator = entries.begin(), endIterator = entries.end();
         iterator != endIterator; ++iterator)
    {
      Complex_Selector_Obj pExtComplexSelector =
      iterator->first; // The selector up to where the @extend is (ie, the thing to merge)
  ...
  static bool parentSuperselector(Complex_Selector_Ptr pOne, Complex_Selector_Ptr pTwo)
  {
    ...
    Element_Selector_Obj fakeParent =
    SASS_MEMORY_NEW(Element_Selector, ParserState("[FAKE]"), "temp");
    Compound_Selector_Obj fakeHead = SASS_MEMORY_NEW(Compound_Selector, ParserState("[FAKE]"), 1 /*size*/);
    fakeHead->elements().push_back(fakeParent);
    Complex_Selector_Obj fakeParentContainer =
    SASS_MEMORY_NEW(Complex_Selector, ParserState("[FAKE]"), Complex_Selector::ANCESTOR_OF,
                    fakeHead /*head*/, NULL /*tail*/);
  ...
  {
    for (NodeDeque::iterator iter = node.collection()->begin(), iterEnd = node.collection()->end();
         iter != iterEnd; iter++)
    {
      Node& child = *iter;
  ...
  */
  void lcs_backtrace(const LCSTable& c,
                     ComplexSelectorDeque& x,
                     ComplexSelectorDeque& y,
                     int i,
                     int j,
                     const LcsCollectionComparator& comparator,
                     ComplexSelectorDeque& out)
  {
    // DEBUG_PRINTLN(LCS, "LCSBACK: X=" << x << " Y=" << y << " I=" << i << " J=" << j)
    // TODO: make printComplexSelectorDeque and use DEBUG_EXEC AND DEBUG_PRINTLN HERE to get
    // equivalent output
  ...
  */
  void lcs_table(const ComplexSelectorDeque& x,
                 const ComplexSelectorDeque& y,
                 const LcsCollectionComparator& comparator,
                 LCSTable& out)
  {
  ...
  */
  void lcs(ComplexSelectorDeque& x, ComplexSelectorDeque& y, const LcsCollectionComparator& comparator, ComplexSelectorDeque& out)
  {
    ...
    return lcs_backtrace(table, x, y, static_cast<int>(x.size()) - 1,
                         static_cast<int>(y.size()) - 1, comparator, out);
  }
  ...
  Node Extend::trim(Node& seqses, bool isReplace)
  {
    ...
    // Normally we use the standard STL iterators, but in this case, we need to access the result
    // collection by index since we're
    // iterating the input collection, computing a value, and then setting the result in the output
    // collection. We have to keep track
    // of the index manually.
    int toTrimIndex = 0;

    for (NodeDeque::iterator seqsesIter = seqses.collection()->begin(),
                             seqsesIterEnd = seqses.collection()->end();
         seqsesIter != seqsesIterEnd; ++seqsesIter)
    {
      ...
      for (NodeDeque::iterator seqs1Iter = seqs1.collection()->begin(),
                               seqs1EndIter = seqs1.collection()->end();
           seqs1Iter != seqs1EndIter; ++seqs1Iter)
      {
        ...
        // Compute the maximum specificity. This requires looking at the "sources" of the sequence.
        // See SimpleSequence.sources in the ruby code
        // for a good description of sources.
        //
        // TODO: I'm pretty sure there's a bug in the sources code. It was implemented for
        // sass-spec's 182_test_nested_extend_loop test.
        // While the test passes, I compared the state of each trim call to verify correctness. The
        // last trim call had incorrect sources. We
        // had an extra source that the ruby version did not have. Without a failing test case, this
        // is going to be extra hard to find. My
        // best guess at this point is that we're cloning an object somewhere and maintaining the
        // sources when we shouldn't be. This is purely
        // a guess though.
        ...
        for (ComplexSelectorSet::iterator sourcesSetIterator = sources.begin(),
                                          sourcesSetIteratorEnd = sources.end();
        ...
        for (NodeDeque::iterator resultIter = result.collection()->begin(),
                                 resultIterEnd = result.collection()->end();
             resultIter != resultIterEnd; ++resultIter)
        {
          ...
          // emulate is: seqs1.equal?(seqs2). equal? is an object comparison, not an equivalency
          // comparision.
          // Since we have the same pointers in seqes and results, we can do a pointer comparision.
          // seqs1 is
          ...
          for (NodeDeque::iterator seqs2Iter = seqs2.collection()->begin(),
                                   seqs2IterEnd = seqs2.collection()->end();
               seqs2Iter != seqs2IterEnd; ++seqs2Iter)
          {
            ...
            DEBUG_PRINTLN(TRIM,
                          "IS SPEC: " << pSeq2->specificity() << " >= " << maxSpecificity
                                      << " " << (pSeq2->specificity() >= maxSpecificity ? "true" : "false"))
            DEBUG_PRINTLN(TRIM,
                          "IS SUPER: " << (pSeq2->is_superselector_of(pSeq1) ? "true" : "false"))
          ...
          // If we found something more specific, we're done. Let the outer loop know and stop
          // iterating.
          if (isMoreSpecificInner)
          {
            isMoreSpecificOuter = true;
  ...
  */
  ...
  static Node chunks(Node& seq1, Node& seq2, const ChunkerType& chunker)
  {
    ...
    firstPermutation.collection()->insert(firstPermutation.collection()->end(),
                                          chunk1.collection()->begin(), chunk1.collection()->end());
    firstPermutation.collection()->insert(firstPermutation.collection()->end(),
                                          chunk2.collection()->begin(), chunk2.collection()->end());
    perms.collection()->push_back(firstPermutation);

    Node secondPermutation = Node::createCollection();
    secondPermutation.collection()->insert(secondPermutation.collection()->end(),
                                           chunk2.collection()->begin(), chunk2.collection()->end());
    secondPermutation.collection()->insert(secondPermutation.collection()->end(),
                                           chunk1.collection()->begin(), chunk1.collection()->end());
    ...
    while (!tail.collection()->empty())
    {
      ...
      do
      {
        ...
        tail.collection()->pop_front();
               ...
               (head.collection()->back().isCombinator() || tail.collection()->front().isCombinator()));
  ...
  */
  static Node mergeFinalOps(Node& seq1, Node& seq2, Node& res)
  {
    ...
    if (ops1.collection()->size() > 1 || ops2.collection()->size() > 1)
    {
      ...
      // If there are multiple operators, something hacky's going on. If one is a supersequence of
      // the other, use that, otherwise give up.
      ...
      {
        res.collection()->insert(res.collection()->begin(), ops1.collection()->rbegin(),
                                 ops1.collection()->rend());
      }
      else
      {
        res.collection()->insert(res.collection()->begin(), ops2.collection()->rbegin(),
                                 ops2.collection()->rend());
      }
    ...
    if (!ops1.collection()->empty() && !ops2.collection()->empty())
    {
      ...
      if (op1.combinator() == Complex_Selector::PRECEDES && op2.combinator() == Complex_Selector::PRECEDES)
      {
        ...
        else
        {
          ...
          Complex_Selector_Obj pMergedWrapper =
          SASS_MEMORY_CLONE(sel1.selector()); // Clone the Complex_Selector to get back to something
                                              // we can transform to a node once we replace the head
                                              // with the unification result
          // TODO: does subject matter? Ruby: return unless merged = sel1.unify(sel2.members,
          // sel2.subject?)
      ...
      else if (((op1.combinator() == Complex_Selector::PRECEDES && op2.combinator() == Complex_Selector::ADJACENT_TO)) ||
               ((op1.combinator() == Complex_Selector::ADJACENT_TO && op2.combinator() == Complex_Selector::PRECEDES)))
      {
        ...
        else
        {
          ...
          Complex_Selector_Obj pMergedWrapper =
          SASS_MEMORY_CLONE(plusSel.selector()); // Clone the Complex_Selector to get back to
                                                 // something we can transform to a node once we
                                                 // replace the head with the unification result
          // TODO: does subject matter? Ruby: merged = plus_sel.unify(tilde_sel.members,
          // tilde_sel.subject?)
          Compound_Selector_Ptr pMerged =
          plusSel.selector()->head()->unify_with(tildeSel.selector()->head());
          pMergedWrapper->head(pMerged);
      ...
      else if (op1.combinator() == Complex_Selector::PARENT_OF &&
               (op2.combinator() == Complex_Selector::PRECEDES || op2.combinator() == Complex_Selector::ADJACENT_TO))
      {
      ...
      else if (op2.combinator() == Complex_Selector::PARENT_OF &&
               (op1.combinator() == Complex_Selector::PRECEDES || op1.combinator() == Complex_Selector::ADJACENT_TO))
      {
    ...
    else if (!ops1.collection()->empty())
    {
      ...
      if (op1.combinator() == Complex_Selector::PARENT_OF && !seq2.collection()->empty() &&
          seq2.collection()->back().selector()->is_superselector_of(seq1.collection()->back().selector()))
      {
        seq2.collection()->pop_back();
      }
    ...
    { // !ops2.collection()->empty()

      Node op2 = ops2.collection()->front();

      if (op2.combinator() == Complex_Selector::PARENT_OF && !seq1.collection()->empty() &&
          seq1.collection()->back().selector()->is_superselector_of(seq2.collection()->back().selector()))
      {
        seq1.collection()->pop_back();
      }
  ...
  */
  Node subweave(Node& one, Node& two)
  {
    ...
    // JMA - filter out the empty nodes (use a new collection, since iterator erase() invalidates
    // the old collection)
    ...
    for (NodeDeque::iterator pathsIter = pathsResult.collection()->begin(),
                             pathsEndIter = pathsResult.collection()->end();
  ...
  */
  Node Extend::weave(Node& path)
  {
    ...
    while (!afters.collection()->empty())
    {
      ...
      for (NodeDeque::iterator beforesIter = befores.collection()->begin(),
                               beforesEndIter = befores.collection()->end();
           beforesIter != beforesEndIter; beforesIter++)
      {
        ...
        for (NodeDeque::iterator subIter = sub.collection()->begin(), subEndIter = sub.collection()->end();
  ...
  */
  /*
   ...
   - What does subject do in?: next unless unified = seq.members.last.unify(self_without_sel,
   subject?)
   - IMPROVEMENT: The search for uniqueness at the end is not ideal since it's has to loop over
   everything...
   - IMPROVEMENT: Check if the final search for uniqueness is doing anything that
   extendComplexSelector isn't already doing...
   */
  template <typename KeyType> class GroupByToAFunctor
  {
    public:
  ...
  Node Extend::extendCompoundSelector(Compound_Selector_Ptr pSelector, CompoundSelectorSet& seen, bool isReplace)
  {
    ...
    */
    ...
    for (SubSetMapResult& groupedPair : arr)
    {
      ...
      // TODO: This can return a Compound_Selector with no elements. Should that just be returning
      // NULL?
      ...
      DEBUG_EXEC(EXTEND_COMPOUND,
                 printCompoundSelector(pSelectorWithoutExtendSelectors, "SELF_WO_SEL: "))
      ...
      pInnermostCompoundSelector->unify_with(pSelectorWithoutExtendSelectors);
      ...
      // TODO: This seems a little fishy to me. See if it causes any problems. From the ruby, we
      // should be able to just
      // get rid of the last Compound_Selector and replace it with this one. I think the reason this
      // code is more
      // complex is that Complex_Selector contains a combinator, but in ruby combinators have
      // already been filtered
      // out and aren't operated on.
      Complex_Selector_Obj pNewSelector = SASS_MEMORY_CLONE(pExtComplexSelector); // ->first();

      Complex_Selector_Obj pNewInnerMost =
      SASS_MEMORY_NEW(Complex_Selector, pSelector->pstate(), Complex_Selector::ANCESTOR_OF,
                      pUnifiedSelector, NULL);
      ...
      if (debugSet.size() > 0)
      {
        ...
        "The new selector should start with no sources. Something needs to be cloned to fix this.");
      }
      debugSet = pExtComplexSelector->sources();
      if (debugSet.size() > 0)
      {
        throw std::runtime_error("The extension selector from our subset map should not have "
                                 "sources. These will bleed to the new selector. Something needs "
                                 "to be cloned to fix this.");
      }
#endif


      // if (pSelector && pSelector->has_line_feed()) pNewInnerMost->has_line_feed(true);
      // Set the sources on our new Complex_Selector to the sources of this simple sequence plus the
      // thing we're extending.
      DEBUG_PRINTLN(EXTEND_COMPOUND, "SOURCES SETTING ON NEW SEQ: " << complexSelectorToNode(pNewSelector))

      DEBUG_EXEC(EXTEND_COMPOUND, ComplexSelectorSet oldSet = pNewSelector->sources();
                 printSourcesSet(oldSet, "SOURCES NEW SEQ BEGIN: "))
      ...
      DEBUG_EXEC(EXTEND_COMPOUND, ComplexSelectorSet newSet = pNewSelector->sources();
                 printSourcesSet(newSet, "SOURCES ON NEW SELECTOR AFTER ADD: "))
      DEBUG_EXEC(EXTEND_COMPOUND,
                 printSourcesSet(pSelector->sources(),
    ...
    for (SubSetMapLookup& pair : holder)
    {
      ...
      for (NodeDeque::iterator iterator = recurseExtendedSelectors.collection()->begin(),
                               endIterator = recurseExtendedSelectors.collection()->end();
           iterator != endIterator; ++iterator)
      {
        ...
        //        << results.contains(newSelector, false /*simpleSelectorOrderDependent*/));
    ...
    while (!hasExtension && pIter)
    {
      ...
      if (pHead)
      {
        ...
        for (SubSetMapPair ext : entries)
        {
          ...
          if (pHead->media_block() && ext.second->media_block()->media_queries() &&
  ...
  Node Extend::extendComplexSelector(Complex_Selector_Ptr selector, CompoundSelectorSet& seen, bool isReplace, bool isOriginal)
  {
    ...
    for (Node& sseqOrOp : *complexSelector.collection())
    {
      ...
      {
        // Wrap our Combinator in two collections to match ruby. This is essentially making a
        // collection Node
        // with one collection child. The collection child represents a Complex_Selector that is
        // only a combinator.
      ...
      // Prepend the Compound_Selector based on the choices logic; choices seems to be extend but
      // with a ruby
  ...
  */
  ...
  Selector_List_Ptr Extend::extendSelectorList(Selector_List_Obj pSelectorList,
                                               bool isReplace,
                                               bool& extendedSomething,
                                               CompoundSelectorSet& seen)
  {

    Selector_List_Obj pNewSelectors =
    SASS_MEMORY_NEW(Selector_List, pSelectorList->pstate(), pSelectorList->length());
    ...
    for (size_t index = 0, length = pSelectorList->length(); index < length; index++)
    {
      ...
      // ruby sass seems to keep a list of things that have extensions and then only extend those.
      // We don't currently do that.
      // Since it's not that expensive to check if an extension exists in the subset map and since
      // it can be relatively expensive to
      // run through the extend code (which does a data model transformation), check if there is
      // anything to extend before doing
      // the extend. We might be able to optimize extendComplexSelector, but this approach keeps us
      // closer to ruby sass (which helps
      // when debugging).
                ...
                else
                {
                  ...
                  for (size_t i = 0; i < ext_sl->length(); i += 1)
                  {
                    if (Complex_Selector_Obj ext_cs = ext_sl->at(i))
                    {
                      ...
                      {
                        Wrapped_Selector_Ptr ext_ws =
                        Cast<Wrapped_Selector>(ext_cs->first()->head()->first());
                        if (ext_ws /* && ext_cs->length() == 1*/)
                        {
                          ...
                          if (!(Cast<Pseudo_Selector>(ws_ss->first()) ||
  ...
  bool shouldExtendBlock(Block_Obj b)
  {
    ...
    //    }
    //
    // We end up visiting two rulesets (one with the selector .a and the other with the selector .a
    // .b).
    // In this case, we don't want to try to pull rules onto .a since they won't get output anyway
    // since
  ...
  // Extend a ruleset by extending the selectors and updating them on the ruleset. The block's rules
  ...
  void Extend::extendObjectWithSelectorAndBlock(Ruleset_Ptr pObject)
  {
    ...
    CompoundSelectorSet seen;
    Selector_List_Obj pNewSelectorList =
    extendSelectorList(pObject->selector(), false, extendedSomething, seen);
  ...
  Extend::Extend(Subset_Map& ssm) : subset_map(ssm)
  {
  }
    ...
    if (b->is_root())
    {
      ...
      for (auto const& it : subset_map.values())
      {
        ...
        error("\"" + str_sel + "\" failed to @extend \"" + str_ext + "\".\n"
                                                                     "The selector \"" +
              str_ext + "\" was not found.\n"
                        "Use \"@extend " +
              str_ext + " !optional\" if the"
                        " extend should be able to fail.",
              (ext ? ext->pstate() : NULL));
      }
    }
  }
