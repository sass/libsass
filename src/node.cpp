...
namespace Sass
{
  ...
  : got_line_feed(false), mType(type), mCombinator(combinator), mpSelector(pSelector),
  ...
  Node Node::klone() const
  {
    ...
    if (mpCollection)
    {
      for (NodeDeque::iterator iter = mpCollection->begin(), iterEnd = mpCollection->end();
           iter != iterEnd; iter++)
      {
        Node& toClone = *iter;
  ...
  bool Node::operator==(const Node& rhs) const
  {
    ...
    else if (this->isCollection())
    {
      ...
      for (NodeDeque::iterator lhsIter = this->collection()->begin(),
                               lhsIterEnd = this->collection()->end(), rhsIter = rhs.collection()->begin();
           lhsIter != lhsIterEnd; lhsIter++, rhsIter++)
      {
    ...
    throw "Comparing unknown node types. A new type was probably added and this method wasn't "
          "implemented for it.";
  }
  ...
  void Node::plus(Node& rhs)
  {
    ...
    this->collection()->insert(this->collection()->end(), rhs.collection()->begin(),
                               rhs.collection()->end());
  }
  ...
  std::ostream& operator<<(std::ostream& os, const Node& node)
  {
    ...
    else if (node.isCollection())
    {
      ...
      for (NodeDeque::iterator iter = node.collection()->begin(), iterBegin = node.collection()->begin(),
                               iterEnd = node.collection()->end();
           iter != iterEnd; iter++)
      {
        if (iter != iterBegin)
        {
  ...
  Complex_Selector_Ptr nodeToComplexSelector(const Node& toConvert)
  {
    ...
    std::string noPath("");
    Complex_Selector_Obj pFirst =
    SASS_MEMORY_NEW(Complex_Selector, ParserState("[NODE]"), Complex_Selector::ANCESTOR_OF, NULL, NULL);
    ...
    for (NodeDeque::iterator childIter = childNodes.begin(), childIterEnd = childNodes.end();
         childIter != childIterEnd; childIter++)
    {
      ...
      else if (child.isCombinator())
      {
        ...
        // if the next node is also a combinator, create another Complex_Selector to hold it so it
        // doesn't replace the current combinator
        if (childIter + 1 != childIterEnd)
        {
          Node& nextNode = *(childIter + 1);
          if (nextNode.isCombinator())
          {
            pCurrent->tail(SASS_MEMORY_NEW(Complex_Selector, ParserState("[NODE]"),
                                           Complex_Selector::ANCESTOR_OF, NULL, NULL));
    ...
    // Put the dummy Compound_Selector in the first position, for consistency with the rest of
    // libsass
    ...
    // pFirst->has_line_feed(pFirst->has_line_feed() || pFirst->tail()->has_line_feed() ||
    // toConvert.got_line_feed);
    pFirst->head(fakeHead);
    return SASS_MEMORY_COPY(pFirst);
  }

  // A very naive trim function, which removes duplicates in a node
  // This is only used in Complex_Selector::unify_with for now, may need modifications to fit other
  // needs
