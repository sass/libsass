...
namespace Sass
{
  ...
  class Token
  {
    ...
    Token() : prefix(0), begin(0), end(0)
    {
    }
    Token(const char* b, const char* e) : prefix(b), begin(b), end(e)
    {
    }
    Token(const char* str) : prefix(str), begin(str), end(str + strlen(str))
    {
    }
    Token(const char* p, const char* b, const char* e) : prefix(p), begin(b), end(e)
    {
    }
    ...
    std::string ws_before() const
    {
      return std::string(prefix, begin);
    }
