...
namespace Sass {
  ...
  Complex_Selector* nodeToComplexSelector(const Node& toConvert, Context& ctx) {
    ...
    string noPath("");
    Position noPosition;
    Complex_Selector* pFirst = new (ctx.mem) Complex_Selector(noPath, noPosition, noPosition, 3100, Complex_Selector::ANCESTOR_OF, NULL, NULL);
      ...
      if (child.isSelector()) {
        ...
        if (childIter+1 != childIterEnd) {
          ...
          if (nextNode.isCombinator()) {
            pCurrent->tail(new (ctx.mem) Complex_Selector(noPath, noPosition, noPosition, 3101, Complex_Selector::ANCESTOR_OF, NULL, NULL));
            pCurrent = pCurrent->tail();
          }
        }
      } else {
    ...
    Compound_Selector* fakeHead = new (ctx.mem) Compound_Selector(noPath, noPosition, noPosition, 3103, 1);
    Selector_Reference* selectorRef = new (ctx.mem) Selector_Reference(noPath, noPosition, noPosition, 3104, NULL);
