...
namespace Sass {
  ...
  static bool parentSuperselector(Complex_Selector* pOne, Complex_Selector* pTwo, Context& ctx) {
    ...
    Position noPosition;
    Type_Selector fakeParent("", noPosition, noPosition, "temp");
    Compound_Selector fakeHead("", noPosition, noPosition, 1 /*size*/);
    fakeHead.elements().push_back(&fakeParent);
    Complex_Selector fakeParentContainer("", noPosition, noPosition, Complex_Selector::ANCESTOR_OF, &fakeHead /*head*/, NULL /*tail*/);
...
//      DEBUG_EXEC(EXTEND_COMPOUND, printComplexSelector(&seq, "SEQ: "))
      ...
      Compound_Selector* pSels = new (ctx.mem) Compound_Selector(pSelector->path(), pSelector->position(), pSelector->closure());
...
//      DEBUG_EXEC(EXTEND_COMPOUND, printCompoundSelector(pSelectorWithoutExtendSelectors, "SELF_WO_SEL: "))
      ...
      if (!pInnermostCompoundSelector) {
        pInnermostCompoundSelector = new (ctx.mem) Compound_Selector(pSelector->path(), pSelector->position(), pSelector->closure());
      }
...
//      DEBUG_EXEC(EXTEND_COMPOUND, printCompoundSelector(pUnifiedSelector, "UNIFIED: "))
      ...
      Complex_Selector* pNewInnerMost = new (ctx.mem) Complex_Selector(pSelector->path(), pSelector->position(), pSelector->closure(), Complex_Selector::ANCESTOR_OF, pUnifiedSelector, NULL);
...
//          DEBUG_PRINTLN(EXTEND_COMPOUND, "ADDING NEW SELECTOR")
  ...
  */
  static Selector_List* extendSelectorList(Selector_List* pSelectorList, Context& ctx, ExtensionSubsetMap& subsetMap, bool& extendedSomething) {
    ...
    Selector_List* pNewSelectors = new (ctx.mem) Selector_List(pSelectorList->path(), pSelectorList->position(), pSelectorList->closure(), pSelectorList->length());
